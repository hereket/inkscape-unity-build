...
constexpr bool CANVAS_ITEM_DEBUG_LOGGING = false;
...
{
    if constexpr (CANVAS_ITEM_DEBUG_LOGGING) {
    ...
    for (auto &item : boost::adaptors::reverse(items)) {
        if constexpr (CANVAS_ITEM_DEBUG_LOGGING) std::cout << "    PICKING: Checking: " << item.get_name() << "  bounds: " << item.get_bounds() << std::endl;
