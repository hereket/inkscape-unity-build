...
enum Dim3 { CANVAS_ITEM_X, CANVAS_ITEM_Y, CANVAS_ITEM_Z };
    ...
    : CanvasItemGrid(group)
{
    ...
    angle_deg[CANVAS_ITEM_X] = 30.0;
    angle_deg[CANVAS_ITEM_Y] = 30.0;
    angle_deg[CANVAS_ITEM_Z] = 0.0;

    angle_rad[CANVAS_ITEM_X] = Geom::rad_from_deg(angle_deg[CANVAS_ITEM_X]);
    angle_rad[CANVAS_ITEM_Y] = Geom::rad_from_deg(angle_deg[CANVAS_ITEM_Y]);
    angle_rad[CANVAS_ITEM_Z] = Geom::rad_from_deg(angle_deg[CANVAS_ITEM_Z]);

    tan_angle[CANVAS_ITEM_X] = std::tan(angle_rad[CANVAS_ITEM_X]);
    tan_angle[CANVAS_ITEM_Y] = std::tan(angle_rad[CANVAS_ITEM_Y]);
    tan_angle[CANVAS_ITEM_Z] = std::tan(angle_rad[CANVAS_ITEM_Z]);
}
...
void CanvasItemGridAxonom::_update(bool)
{
    ...
    spacing_ylines = lyw / (tan_angle[CANVAS_ITEM_X] + tan_angle[CANVAS_ITEM_Z]);
    lxw_x          = Geom::are_near(tan_angle[CANVAS_ITEM_X], 0) ? Geom::infinity() : lyw / tan_angle[CANVAS_ITEM_X];
    lxw_z          = Geom::are_near(tan_angle[CANVAS_ITEM_Z], 0) ? Geom::infinity() : lyw / tan_angle[CANVAS_ITEM_Z];
...
{
    defer([=, this] {
        angle_deg[CANVAS_ITEM_X] = std::clamp(deg, 0.0, 89.0); // setting to 90 and values close cause extreme slowdowns
        angle_rad[CANVAS_ITEM_X] = Geom::rad_from_deg(angle_deg[CANVAS_ITEM_X]);
        tan_angle[CANVAS_ITEM_X] = std::tan(angle_rad[CANVAS_ITEM_X]);
        request_update();
    });
...
{
    defer([=, this] {
        angle_deg[CANVAS_ITEM_Z] = std::clamp(deg, 0.0, 89.0); // setting to 90 and values close cause extreme slowdowns
        angle_rad[CANVAS_ITEM_Z] = Geom::rad_from_deg(angle_deg[CANVAS_ITEM_Z]);
        tan_angle[CANVAS_ITEM_Z] = std::tan(angle_rad[CANVAS_ITEM_Z]);
        request_update();
    });
...
void CanvasItemGridAxonom::_render(Inkscape::CanvasItemBuffer &buf) const
{
    ...
    double const xintercept_y_bc = (buf_tl_gc.x() * tan_angle[CANVAS_ITEM_X]) - buf_tl_gc.y();
    double const xstart_y_sc = (xintercept_y_bc - std::floor(xintercept_y_bc / lyw) * lyw) + buf.rect.top();
    int const xlinestart = std::round((xstart_y_sc - buf_tl_gc.x() * tan_angle[CANVAS_ITEM_X] - ow.y()) / lyw);
    ...
    for (double y = xstart_y_sc; y < buf.rect.bottom(); y += lyw, xlinenum++) {
        ...
        int x1 = x0 + round((buf.rect.bottom() - y) / tan_angle[CANVAS_ITEM_X]);
        int y1 = buf.rect.bottom();
        if (Geom::are_near(tan_angle[CANVAS_ITEM_X], 0)) {
            x1 = buf.rect.right();
            y1 = y0;
        }
    ...
    if (!Geom::are_near(tan_angle[CANVAS_ITEM_X], 0)) {
        double const xstart_x_sc = buf.rect.left() + (lxw_x - (xstart_y_sc - buf.rect.top()) / tan_angle[CANVAS_ITEM_X]);
        ...
        for (double x = xstart_x_sc; x < buf.rect.right(); x += lxw_x, xlinenum--) {
            ...
            int const x1 = x0 + round((y1 - y0) / tan_angle[CANVAS_ITEM_X]);
    ...
    double const zintercept_y_bc = (buf_tl_gc.x() * -tan_angle[CANVAS_ITEM_Z]) - buf_tl_gc.y();
    double const zstart_y_sc = (zintercept_y_bc - std::floor(zintercept_y_bc / lyw) * lyw) + buf.rect.top();
    int const  zlinestart = std::round((zstart_y_sc + buf_tl_gc.x() * tan_angle[CANVAS_ITEM_Z] - ow.y()) / lyw);
    ...
    for (double y = zstart_y_sc; y < buf.rect.bottom(); y += lyw, zlinenum++, next_y = y) {
        int const x0 = buf.rect.left();
        int const y0 = round(y);
        int x1 = x0 + round((y - buf.rect.top()) / tan_angle[CANVAS_ITEM_Z]);
        int y1 = buf.rect.top();
        if (Geom::are_near(tan_angle[CANVAS_ITEM_Z], 0)) {
            x1 = buf.rect.right();
            y1 = y0;
        }
    ...
    if (!Geom::are_near(tan_angle[CANVAS_ITEM_Z], 0)) {
        double const zstart_x_sc = buf.rect.left() + (next_y - buf.rect.bottom()) / tan_angle[CANVAS_ITEM_Z];
        for (double x = zstart_x_sc; x < buf.rect.right(); x += lxw_z, zlinenum++) {
            int const y0 = buf.rect.bottom();
            int const y1 = buf.rect.top();
            int const x0 = round(x);
            int const x1 = x0 + round(buf.rect.height() / tan_angle[CANVAS_ITEM_Z]);
