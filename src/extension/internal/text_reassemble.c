...
int trinfo_append_out(TR_INFO *tri, const char *src){
   ...
   if(tri->outused + (int) slen + 1 >= tri->outspace){
      ...
      tmp = (uint8_t*)realloc(tri->out, tri->outspace * sizeof(uint8_t) );
      if(tmp){ tri->out = tmp; }
      else { return(-1); }
   }
...
void TR_layout_2_svg(TR_INFO *tri){
   ...
   */
   ...
   char *hold_locale = (char*)malloc(sizeof(char) * (strlen(prev_locale) + 1));
         ...
         */
         ...
         if(tspj->ykern == 0.0){
            ...
            if((ldir == LDIR_RL && (spaces <= -0.9 && spaces >= -2.1)) ||
               ...
               uint8_t *newstring = (uint8_t*)malloc(1 + ispaces + slen);
