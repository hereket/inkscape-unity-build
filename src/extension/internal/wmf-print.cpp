...
/* globals */
static double       WMF_PX2WORLD;   // value set in begin()
static bool         WMF_FixPPTCharPos, WMF_FixPPTDashLine, WMF_FixPPTGrad2Polys, WMF_FixPPTPatternAsHatch;
...
unsigned int PrintWmf::begin(Inkscape::Extension::Print *mod, SPDocument *doc)
{
    ...
    // Typically WMF_PX2WORLD is 1200/90, using inkscape's default dpi
    WMF_PX2WORLD = 1200.0 / Inkscape::Util::Quantity::convert(1.0, "in", "px");
    WMF_FixPPTCharPos        = mod->get_param_bool("FixPPTCharPos");
    WMF_FixPPTDashLine       = mod->get_param_bool("FixPPTDashLine");
    WMF_FixPPTGrad2Polys     = mod->get_param_bool("FixPPTGrad2Polys");
    WMF_FixPPTPatternAsHatch = mod->get_param_bool("FixPPTPatternAsHatch");
...
int PrintWmf::create_pen(SPStyle const *style, const Geom::Affine &transform)
{
    ...
    if (style) {  // override some or all of the preceding
        ...
        linewidth = MAX(1, (uint32_t) round(scale * style->stroke_width.computed * WMF_PX2WORLD));
        ...
        if (!style->stroke_dasharray.values.empty()) {
            if (!WMF_FixPPTDashLine) { // if this is set code elsewhere will break dots/dashes into many smaller lines.
    ...
    if (create_brush(style, nullptr)) {
        ...
        */
        ...
        if (
            ...
            (!style->stroke_dasharray.values.empty()  && WMF_FixPPTDashLine)             ||
            !all_closed
        ) {
    ...
    Geom::OptRect const &/*pbox*/, Geom::OptRect const &/*dbox*/, Geom::OptRect const &/*bbox*/)
{
    ...
    if (!style->stroke_dasharray.values.empty()  && WMF_FixPPTDashLine) {
...
bool PrintWmf::print_simple_shape(Geom::PathVector const &pathv, const Geom::Affine &transform)
{
    ...
    for (const auto & pit : pv) {
        ...
        p0[X] = (p0[X] * WMF_PX2WORLD);
        p0[Y] = (p0[Y] * WMF_PX2WORLD);
        ...
        for (Geom::Path::const_iterator cit = pit.begin(); cit != pit.end_open(); ++cit) {
            if (is_straight_curve(*cit)) {
                ...
                //p0[X] = (p0[X] * WMF_PX2WORLD);
                p1[X] = (p1[X] * WMF_PX2WORLD);
                //p0[Y] = (p0[Y] * WMF_PX2WORLD);
                p1[Y] = (p1[Y] * WMF_PX2WORLD);
                ...
                //p0[X] = (p0[X] * WMF_PX2WORLD);
                p1[X] = (p1[X] * WMF_PX2WORLD);
                p2[X] = (p2[X] * WMF_PX2WORLD);
                p3[X] = (p3[X] * WMF_PX2WORLD);
                //p0[Y] = (p0[Y] * WMF_PX2WORLD);
                p1[Y] = (p1[Y] * WMF_PX2WORLD);
                p2[Y] = (p2[Y] * WMF_PX2WORLD);
                p3[Y] = (p3[Y] * WMF_PX2WORLD);
    ...
    SPStyle const * /*style*/)  /** provides indirect link to image object */
{
    ...
    */
    ...
    U_POINT16 Dest  = point16_set(round(pLL2[Geom::X] * WMF_PX2WORLD), round(pLL2[Geom::Y] * WMF_PX2WORLD));
    U_POINT16 cDest = point16_set(round(pWH2[Geom::X] * WMF_PX2WORLD), round(pWH2[Geom::Y] * WMF_PX2WORLD));
                            ...
                            SPStyle const *const style)
{
    ...
    int rtl = 0;
    int16_t *adx;
    smuggle_adxky_out(text, &adx, &ky, &rtl, &ndx, WMF_PX2WORLD * std::min(tf.expansionX(), tf.expansionY())); // side effect: free() adx
    ...
    */
    int textheight = round(-style->font_size.computed * WMF_PX2WORLD * std::min(tf.expansionX(), tf.expansionY()));
    if (!hfont) {
    ...
    */
    ...
    p2[Geom::X] = (p2[Geom::X] * WMF_PX2WORLD);
    p2[Geom::Y] = (p2[Geom::Y] * WMF_PX2WORLD);
