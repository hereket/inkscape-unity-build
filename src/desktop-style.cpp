 ...
 */
void
sp_desktop_set_color(SPDesktop *desktop, Inkscape::Color const &color, bool is_relative, bool fill)
{
...
sp_desktop_get_color(SPDesktop *desktop, bool is_fill)
{
    ...
    return Inkscape::Color::parse(desktop->current ? property : nullptr);
}
...
sp_desktop_get_color_tool(SPDesktop *desktop, Glib::ustring const &tool, bool is_fill)
{
    ...
    std::optional<Inkscape::Color> ret;
    if (css) {
        gchar const *property = sp_repr_css_property(css, is_fill ? "fill" : "stroke", "#000");
        // if there is style and the property in it,
        ret = Inkscape::Color::parse(desktop->current ? property : nullptr);
        sp_repr_css_attr_unref(css);
    }
    return ret;
}
