...
void SPOffset::release() {
    ...
    if (this->originalPath) {
    	delete ((livarot::Path *) this->originalPath);
    }
    ...
    switch (key)
    {
        ...
        case SPAttr::SODIPODI_ORIGINAL:
            if (value == nullptr) {
                ...
                this->originalPath = new livarot::Path;
                reinterpret_cast<livarot::Path *>(this->originalPath)->LoadPathVector(pv);
...
void SPOffset::set_shape() {
    ...
    livarot::Path *orig = new livarot::Path;
    orig->Copy ((livarot::Path *)this->originalPath);

    if ( use_slow_but_correct_offset_method == false ) {
        // version par outline
        Shape *theShape = new Shape;
        Shape *theRes = new Shape;
        livarot::Path *originaux[1];
        ...
        int    nbPart=0;
        livarot::Path** parts=res->SubPaths(nbPart,true);
...
{
    if (offset == nullptr || offset->originalPath == nullptr || ((livarot::Path *) offset->originalPath)->descr_cmd.size() <= 1) {
        return 1.0;
    }
     ...
     */
    // move
    ((livarot::Path *) offset->originalPath)->Convert (1.0);
    ((livarot::Path *) offset->originalPath)->Fill (theShape, 0);
...
sp_offset_top_point (SPOffset const * offset, Geom::Point *px)
{
    ...
    livarot::Path *finalPath = new livarot::Path;
