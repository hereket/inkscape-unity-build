...
void refresh_textpath_source(SPTextPath* tp)
{
    ...
    if ( tp->sourcePath->originalPath ) {
        ...
        tp->originalPath = new livarot::Path;
...
void sp_textpath_to_text(SPObject *tp)
{
    ...
    livarot::Path *path = cast<SPTextPath>(tp)->originalPath;
    ...
    int unused = 0;
    livarot::Path::cut_position *cut_pos = path->CurvilignToPosition(1, &offset, unused);
    Geom::Point midpoint;
    Geom::Point tangent;
