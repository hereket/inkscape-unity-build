...
std::unique_ptr<Shape> SPText::getExclusionShape() const
{
    ...
    for (auto *href : style->shape_subtract.hrefs) {
        ...
        auto temp = std::make_unique<livarot::Path>();
        temp->LoadPathVector(curve->get_pathvector(), shape->getRelativeTransform(this), true);

        auto margin = std::make_unique<livarot::Path>();
...
std::unique_ptr<Shape> SPText::getInclusionShape(SPShape *shape) const
{
    ...
    auto temp_path = std::make_unique<livarot::Path>();
    temp_path->LoadPathVector(pathvector, shape->transform, true);

    auto const make_nice_shape = [](std::unique_ptr<livarot::Path> const &contour) -> std::unique_ptr<Shape> {
    ...
    if (padding) {
        auto outline = std::make_unique<livarot::Path>();
