...
void SPRect::set_shape() {
    ...
    if ((rx > 1e-18) && (ry > 1e-18)) {
        ...
        c.curveto(x + w - rx * (1 - SP_C1), y, x + w, y + ry * (1 - SP_C1), x + w, y + ry);

        if (ry < h2) {
            c.lineto(x + w, y + h - ry);
        }

        c.curveto(x + w, y + h - ry * (1 - SP_C1), x + w - rx * (1 - SP_C1), y + h, x + w - rx, y + h);

        if (rx < w2) {
            c.lineto(x + rx, y + h);
        }

        c.curveto(x + rx * (1 - SP_C1), y + h, x, y + h - ry * (1 - SP_C1), x, y + h - ry);

        if (ry < h2) {
            c.lineto(x, y + ry);
        }

        c.curveto(x, y + ry * (1 - SP_C1), x + rx * (1 - SP_C1), y, x + rx, y);
    } else {
