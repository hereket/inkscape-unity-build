...
bool is_item_s(SPObject const &object) {
...
void SPItem::raiseToTop() {
    ...
    auto topmost = std::find_if(list.rbegin(), end, &is_item_s);
    // auto topmost = find_last_if(++parent->children.iterator_to(*this), parent->children.end(), &is_item_s);
...
bool SPItem::raiseOne() {
    auto next_higher = std::find_if(++parent->children.iterator_to(*this), parent->children.end(), &is_item_s);
...
bool SPItem::lowerOne() {
    ...
    auto next_lower = std::find_if(start, list.rend(), &is_item_s);
...
void SPItem::lowerToBottom() {
    auto bottom = std::find_if(parent->children.begin(), parent->children.iterator_to(*this), &is_item_s);
                 ...
                 SP_OBJECT_STYLE_MODIFIED_FLAG))
    {
        ...
        if (flags & SP_OBJECT_STYLE_MODIFIED_FLAG) {
            for (auto &v : views) {
                ...
                v.drawingitem->setAntialiasing(style->shape_rendering.computed == SP_CSS_SHAPE_RENDERING_CRISPEDGES ? Inkscape::Antialiasing::ANone : Inkscape::Antialiasing::Good);
