...
static int grid_calculate_scaling_factor(double length, int major)
{
    int multiply = 1;
    ...
    for (int index = start; index < limit; ++index) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (getSnapVisibleOnly() && desktop) {
                ...
                int const mult = grid_calculate_scaling_factor(sw.length(), _grid->getMajorLineInterval());
                scaled_spacing *= mult;
            }
...
LineSnapper::LineList GridSnapper::getSnapLinesAxonom(Geom::Point const &p) const
{
    ...
    if (getSnapVisibleOnly() && desktop) {
        ...
        int const mult = grid_calculate_scaling_factor(lyw, _grid->getMajorLineInterval());
        spacing_h *= mult;
        spacing_v *= mult;
    }
