...
bool Parsers::_parse(std::istringstream &ss, Space::Type &type, std::string &name, std::vector<double> &values,
                     std::vector<double> &fallback) const
{
    auto ptype = ColorParser::getCssPrefix(ss);
 ...
 */
void Parsers::addParser(ColorParser *parser)
{
    // Map parsers by their expected prefix for quicker lookup
    auto const [it, inserted] = _parsers.emplace(parser->getPrefix(), std::vector<std::shared_ptr<ColorParser>>{});
 ...
 */
std::string ColorParser::parseColor(std::istringstream &ss, std::vector<double> &output, bool &more) const
{
 ...
 */
std::string ColorParser::getCssPrefix(std::istringstream &ss)
{
    std::string token;
    ss >> std::ws;
    if (ss.peek() == '#') {
 ...
 */
bool ColorParser::css_number(std::istringstream &ss, double &value, std::string &unit, bool &end, char const sep)
{
 ...
 */
bool ColorParser::append_css_value(std::istringstream &ss, std::vector<double> &output, bool &end, char const sep,
                              double scale)
{
    double value;
    std::string unit;
