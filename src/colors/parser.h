...
{
public:
    ColorParser(std::string prefix, Space::Type type)
...
class LegacyParser : public ColorParser
{
public:
    LegacyParser(std::string const &prefix, Space::Type type, bool alpha)
        : ColorParser(alpha ? prefix + "a" : prefix, type)
        , _alpha(alpha)
    {}
...
protected:
    ...
    void addParser(ColorParser *parser);

private:
    bool _parse(std::istringstream &ss, Space::Type &type, std::string &cms, std::vector<double> &values,
                std::vector<double> &fallback) const;

    std::map<std::string, std::vector<std::shared_ptr<ColorParser>>> _parsers;
};
