 ...
 */
constexpr double OKLCH_CHROMA_SCALE = 0.4;
constexpr double OKCLH_HUE_SCALE = 360;
...
void OkLch::toOkLab(std::vector<double> &in_out)
{
    ...
    double c = in_out[1];
    Geom::sincos(Geom::Angle::from_degrees(in_out[2] * OKCLH_HUE_SCALE), in_out[2], in_out[1]);
    in_out[1] *= c;
    in_out[2] *= c;
}
...
void OkLch::fromOkLab(std::vector<double> &in_out)
{
    ...
    if (c > 0.001) {
        ...
        in_out[2] = Geom::deg_from_rad(hue_angle.radians0()) / OKCLH_HUE_SCALE;
    } else {
        in_out[2] = 0;
    }
    in_out[1] = c;
...
bool OkLch::Parser::parse(std::istringstream &ss, std::vector<double> &output) const
{
    ...
    if (append_css_value(ss, output, end, ',')                  // Luminance
        && append_css_value(ss, output, end, ',', OKLCH_CHROMA_SCALE) // Chroma
        && append_css_value(ss, output, end, '/', OKCLH_HUE_SCALE)    // Hue
...
std::string OkLch::toString(std::vector<double> const &values, bool opacity) const
{
    ...
    os << values[0]                // Luminance
       << values[1] * OKLCH_CHROMA_SCALE // Chroma
       << values[2] * OKCLH_HUE_SCALE;   // Hue
