 ...
 */
constexpr double OKLAB_MIN_SCALE = -0.4;
constexpr double OKLAB_MAX_SCALE = 0.4;

void OkLab::scaleUp(std::vector<double> &in_out)
{
    in_out[1] = SCALE_UP(in_out[1], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE);
    in_out[2] = SCALE_UP(in_out[2], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE);
}

void OkLab::scaleDown(std::vector<double> &in_out)
{
    in_out[1] = SCALE_DOWN(in_out[1], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE);
    in_out[2] = SCALE_DOWN(in_out[2], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE);
}
...
bool OkLab::Parser::parse(std::istringstream &ss, std::vector<double> &output) const
{
    ...
    if (append_css_value(ss, output, end, ',')               // Luminance
        && append_css_value(ss, output, end, ',', OKLAB_MAX_SCALE) // Chroma A
        && append_css_value(ss, output, end, '/', OKLAB_MAX_SCALE) // Chroma B
...
std::string OkLab::toString(std::vector<double> const &values, bool opacity) const
{
    ...
    os << values[0]                                  // Luminance
       << SCALE_UP(values[1], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE)  // Chroma A
       << SCALE_UP(values[2], OKLAB_MIN_SCALE, OKLAB_MAX_SCALE); // Chroma B
