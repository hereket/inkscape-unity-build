 ...
 * allocate a new node
 */
Ocnode *ocnodeNew(trace::Pool<Ocnode> &pool)
{
...
void ocnodeFree(trace::Pool<Ocnode> &pool, Ocnode *node)
{
    pool.drop(node);
}
 ...
 * free a full octree
 */
void octreeDelete(trace::Pool<Ocnode> &pool, Ocnode *node)
{
    if (!node) return;
    for (auto &i : node->child) {
        octreeDelete(pool, i);
 ...
 */
void ocnodeLeaf(trace::Pool<Ocnode> &pool, Ocnode **ref, RGB rgb)
{
    assert(ref);
 ...
 */
int octreeMerge(trace::Pool<Ocnode> &pool, Ocnode *parent, Ocnode **ref, Ocnode *node1, Ocnode *node2)
{
    assert(ref);
 ...
 */
void ocnodeStrip(trace::Pool<Ocnode> &pool, Ocnode **ref, int &count, unsigned long lvl)
{
    Ocnode *node = *ref;
    if (!node) return;
    assert(ref == node->ref);
 ...
 */
void octreePrune(trace::Pool<Ocnode> &pool, Ocnode **ref, int ncolor)
{
    assert(ref);
    assert(ncolor > 0);
 ...
 */
void octreeBuildArea(trace::Pool<Ocnode> &pool, RgbMap const &rgbmap, Ocnode **ref, int x1, int y1, int x2, int y2, int ncolor)
{
 ...
 */
Ocnode *octreeBuild(trace::Pool<Ocnode> &pool, RgbMap const &rgbmap, int ncolor)
{
    // create the octree
    Ocnode *node = nullptr;
