...
void  livarot::Path::DashPolyline(float head,float tail,float body,int nbD, const float dashs[],bool stPlain,float stOffset)
{
...
void  livarot::Path::DashPolylineFromStyle(SPStyle *style, float scale, float min_len)
{
...
void livarot::Path::DashSubPath(int spL, int spP, std::vector<path_lineto> const &orig_pts, float head,float tail,float body,int nbD, const float dashs[],bool stPlain,float stOffset)
{
 ...
 */
Geom::PathVector livarot::Path::MakePathVector() const
{
    Geom::PathVector pv;
...
void livarot::Path::AddCurve(Geom::Curve const &c)
{
 ...
 */
void  livarot::Path::LoadPath(Geom::Path const &path, Geom::Affine const &tr, bool doTransformation, bool append)
{
    if (!append) {
        SetBackData (false);
        Reset();
...
void  livarot::Path::LoadPathVector(Geom::PathVector const &pv)
{
    LoadPathVector(pv, Geom::Affine(), false);
}

void livarot::Path::LoadPathVector(Geom::PathVector const &pv, std::vector<Geom::PathVectorTime> const &cuts)
{
    SetBackData(false);
    Reset();
...
void  livarot::Path::LoadPathVector(Geom::PathVector const &pv, Geom::Affine const &tr, bool doTransformation)
{
    SetBackData (false);
    Reset();
...
livarot::Path**      livarot::Path::SubPaths(int &outNb,bool killNoSurf)
{
  int      nbRes=0;
  livarot::Path**   res=nullptr;
  livarot::Path*    curAdd=nullptr;
        ...
        if ( curAdd ) {
          if ( curAdd->descr_cmd.size() > 1 ) {
            ...
            if ( fabs(addSurf) > 0.0001 || killNoSurf == false ) {
              res=(livarot::Path**)g_realloc(res,(nbRes+1)*sizeof(livarot::Path*));
              res[nbRes++]=curAdd;
            } else { 
              delete curAdd;
            }
...
livarot::Path**      livarot::Path::SubPathsWithNesting(int &outNb,bool killNoSurf,int nbNest,int* nesting,int* conts)
{
  int      nbRes=0;
  livarot::Path**   res=nullptr;
  livarot::Path*    curAdd=nullptr;
  ...
  for (int i=0;i<int(descr_cmd.size());i++) {
    ...
    switch ( typ ) {
      case descr_moveto:
      {
        ...
        livarot::Path*  hasParent=nullptr;
...
void livarot::Path::ConvertForcedToVoid()
{  
    for (int i=0; i < int(descr_cmd.size()); i++) {
...
void livarot::Path::ConvertForcedToMoveTo()
{  
...
static int       CmpPosition(const void * p1, const void * p2) {
    livarot::Path::cut_position *cp1=(livarot::Path::cut_position*)p1;
    livarot::Path::cut_position *cp2=(livarot::Path::cut_position*)p2;
...
livarot::Path::cut_position* livarot::Path::CurvilignToPosition(int nbCv, double *cvAbs, int &nbCut)
{
...
livarot::Path::cut_position livarot::Path::PointToCurvilignPosition(Geom::Point const &pos, unsigned seg) const
{
    ...
    livarot::Path::cut_position result;
    if (bestSeg == 0) {
        result.piece = 0;
        result.t = 0.0;
    } else {
...
*/
double livarot::Path::PositionToLength(int piece, double t)
{
    double length = 0.0;
    for (unsigned i = 1 ; i < pts.size() ; i++) {
...
void livarot::Path::ConvertPositionsToForced(int nbPos, cut_position *poss)
{
    if ( nbPos <= 0 ) {
        return;
    }
...
void        livarot::Path::ConvertPositionsToMoveTo(int nbPos,cut_position* poss)
{
  ConvertPositionsToForced(nbPos,poss);
//  ConvertForcedToMoveTo();
  // on fait une version customizee a la place

  livarot::Path*  res=new livarot::Path;
