...
void livarot::Path::ConvertWithBackData(double treshhold, bool relative)
{
...
void livarot::Path::Convert(double treshhold)
{
    if ( descr_flags & descr_doing_subpath ) {
        CloseSubpath();
    }
...
void livarot::Path::ConvertEvenLines(double treshhold)
{
    if ( descr_flags & descr_doing_subpath ) {
        CloseSubpath();
    }
...
const Geom::Point livarot::Path::PrevPoint(int i) const
{
...
void livarot::Path::CubicTangent(double t, Geom::Point &oPt, const Geom::Point &iS, const Geom::Point &isD,
...
void livarot::Path::ArcAngles(const Geom::Point &iS, const Geom::Point &iE,
...
void livarot::Path::DoArc(Geom::Point const &iS, Geom::Point const &iE,
...
void livarot::Path::RecCubicTo( Geom::Point const &iS, Geom::Point const &isD,
...
void livarot::Path::Fill(Shape* dest, int pathID, bool justAdd, bool closeIfNeeded, bool invert)
{
    if ( dest == nullptr ) {
        return;
    }
