...
void livarot::Path::Outline(livarot::Path *dest, double width, JoinType join, ButtType butt, double miter)
{
    ...
    livarot::Path *rev = new livarot::Path;
...
void
livarot::Path::OutsideOutline (livarot::Path * dest, double width, JoinType join, ButtType butt,
                      double miter)
{
	if (descr_flags & descr_doing_subpath) {
		CloseSubpath();
...
void
livarot::Path::InsideOutline (livarot::Path * dest, double width, JoinType join, ButtType butt,
                     double miter)
{
	if ( descr_flags & descr_doing_subpath ) {
		CloseSubpath();
...
void livarot::Path::SubContractOutline(int off, int num_pd,
                              livarot::Path *dest, outline_callbacks & calls,
...
bool
livarot::Path::IsNulCurve (std::vector<PathDescr*> const &cmd, int curD, Geom::Point const &curX)
{
...
void livarot::Path::TangentOnSegAt(double at, Geom::Point const &iS, PathDescrLineTo const &fin,
...
// barf
void livarot::Path::TangentOnArcAt(double at, const Geom::Point &iS, PathDescrArcTo const &fin,
...
void
livarot::Path::TangentOnCubAt (double at, Geom::Point const &iS, PathDescrCubicTo const &fin, bool before,
                      Geom::Point &pos, Geom::Point &tgt, double &len, double &rad)
{
...
void
livarot::Path::OutlineJoin (livarot::Path * dest, Geom::Point pos, Geom::Point stNor, Geom::Point enNor, double width,
...
void
livarot::Path::RecStdCubicTo (outline_callback_data * data, double tol, double width,
                     int lev)
{
...
void
livarot::Path::StdCubicTo (livarot::Path::outline_callback_data * data, double tol, double width)
{
//	fflush (stdout);
...
void
livarot::Path::RecStdArcTo (outline_callback_data * data, double tol, double width,
                   int lev)
{
	Geom::Point stPos, miPos, enPos;
	Geom::Point stTgt, enTgt, miTgt, stNor, enNor, miNor;
...
void
livarot::Path::StdArcTo (livarot::Path::outline_callback_data * data, double tol, double width)
{
	data->d.a.stA = 0.0;
	data->d.a.enA = 1.0;
