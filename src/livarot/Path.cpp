...
void livarot::Path::Copy(livarot::Path * who)
{
    ResetPoints();
...
void livarot::Path::CloseSubpath()
{
    descr_flags &= ~(descr_doing_subpath);
}

int livarot::Path::ForcePoint()
{
...
void livarot::Path::InsertForcePoint(int at)
{
...
int livarot::Path::MoveTo(Geom::Point const &iPt)
{
    if ( descr_flags & descr_doing_subpath ) {
	CloseSubpath();
    }
...
void livarot::Path::InsertMoveTo(Geom::Point const &iPt, int at)
{
    if ( at < 0 || at > int(descr_cmd.size()) ) {
        return;
    }
...
int livarot::Path::LineTo(Geom::Point const &iPt)
{
    if (!( descr_flags & descr_doing_subpath )) {
	return MoveTo (iPt);
    }
...
void livarot::Path::InsertLineTo(Geom::Point const &iPt, int at)
{
    if ( at < 0 || at > int(descr_cmd.size()) ) {
        return;
    }
...
int livarot::Path::CubicTo(Geom::Point const &iPt, Geom::Point const &iStD, Geom::Point const &iEnD)
{
    if ( (descr_flags & descr_doing_subpath) == 0) {
	return MoveTo (iPt);
    }
...
void livarot::Path::InsertCubicTo(Geom::Point const &iPt, Geom::Point const &iStD, Geom::Point const &iEnD, int at)
{
    if ( at < 0 || at > int(descr_cmd.size()) ) {
	return;
    }
...
int livarot::Path::ArcTo(Geom::Point const &iPt, double iRx, double iRy, double angle,
...
void livarot::Path::InsertArcTo(Geom::Point const &iPt, double iRx, double iRy, double angle,
 ...
 */
void
livarot::Path::SetBackData (bool nVal)
{
	if (! back) {
		if (nVal) {
			back = true;
...
void livarot::Path::ResetPoints()
{
    pts.clear();
}


int livarot::Path::AddPoint(Geom::Point const &iPt, bool mvto)
{
    if (back) {
...
int livarot::Path::AddPoint(Geom::Point const &iPt, int ip, double it, bool mvto)
{
    if (! back) {
...
int livarot::Path::AddForcedPoint()
{
...
void livarot::Path::PolylineBoundingBox(double &l, double &t, double &r, double &b)
{
  l = t = r = b = 0.0;
  if ( pts.empty() ) {
      return;
...
void livarot::Path::PointAt(int piece, double at, Geom::Point &pos)
{
...
void livarot::Path::PointAndTangentAt(int piece, double at, Geom::Point &pos, Geom::Point &tgt)
{
    if (piece < 0 || piece >= int(descr_cmd.size())) {
 ...
 */
void livarot::Path::Transform(const Geom::Affine &trans)
{
    LoadPathVector(MakePathVector() * trans);
}

void livarot::Path::FastBBox(double &l,double &t,double &r,double &b)
{
    l = t = r = b = 0;
    bool empty = true;
...
std::string livarot::Path::svg_dump_path() const
{
...
bool livarot::Path::IsLineSegment(int piece)
{
    if (piece < 0 || piece >= int(descr_cmd.size())) {
        return false;
    }
