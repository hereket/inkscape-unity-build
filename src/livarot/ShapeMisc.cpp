...
void
Shape::ConvertToForme (livarot::Path * dest)
{
...
void Shape::ConvertToForme(livarot::Path *dest, int nbP, livarot::Path *const *orig, bool never_split)
{
...
void Shape::ConvertToFormeNested(livarot::Path *dest, int nbP, livarot::Path *const *orig, int &nbNest, int *&nesting, int *&contStart, bool never_split)
{
  nesting=nullptr;
  contStart=nullptr;
  nbNest=0;
    ...
    if ( a->_has_back_data ) {
      if ( a->ebData[i].pathID >= 0 && a->ebData[stB].pathID == a->ebData[i].pathID && a->ebData[stB].pieceID == a->ebData[i].pieceID
		...
		if (mode == tweak_mode_push || mode == tweak_mode_repel || mode == tweak_mode_roughen) {
            livarot::Path::DoLeftJoin (this, 0, join, ptP+this_vec, stD+this_vec, seD+this_vec, miter, stL, seL,
												stNo, enNo,usePathID,usePieceID,useT);
			a->swsData[i].stPt = enNo;
			a->swsData[stB].enPt = stNo;
		} else {
			if (power > 0) {
                livarot::Path::DoRightJoin (this, this_power * scaler, join, ptP, stD, seD, miter, stL, seL,
													 stNo, enNo,usePathID,usePieceID,useT);
				a->swsData[i].stPt = enNo;
				a->swsData[stB].enPt = stNo;
			} else {
                livarot::Path::DoLeftJoin (this, -this_power * scaler, join, ptP, stD, seD, miter, stL, seL,
													stNo, enNo,usePathID,usePieceID,useT);
				a->swsData[i].stPt = enNo;
				a->swsData[stB].enPt = stNo;
			}
...
Shape::MakeOffset (Shape * a, double dec, JoinType join, double miter, bool do_profile, double cx, double cy, double radius, Geom::Affine *i2doc)
{
  ...
  for (int i = 0; i < a->numberOfEdges(); i++)
  {
    ...
    if (dec > 0)
    {
        livarot::Path::DoRightJoin (this, this_dec, join, ptP, stD, seD, miter, stL, seL,
                         stNo, enNo,usePathID,usePieceID,useT);
      a->swsData[i].stPt = enNo;
      a->swsData[stB].enPt = stNo;
    }
    ...
    {
        livarot::Path::DoLeftJoin (this, -this_dec, join, ptP, stD, seD, miter, stL, seL,
                        stNo, enNo,usePathID,usePieceID,useT);
      a->swsData[i].stPt = enNo;
      a->swsData[stB].enPt = stNo;
    }
...
void Shape::AddContour(livarot::Path *dest, int num_orig, livarot::Path *const *orig, int start_edge, bool never_split)
{
    int edge = start_edge;
...
int Shape::ReFormeLineTo(int bord, livarot::Path *dest, bool never_split)
{
...
int Shape::ReFormeArcTo(int bord, livarot::Path *dest, livarot::Path *from, bool never_split)
{
  int nPiece = ebData[bord].pieceID;
  int nPath = ebData[bord].pathID;
  ...
  livarot::Path::ArcAngles (from->PrevPoint (nPiece - 1), nData->p,nData->rx,nData->ry,nData->angle*M_PI/180.0, nLarge, nClockwise,  sang, eang);
  if (nClockwise)
  {
    if (sang < eang)
      sang += 2 * M_PI;
...
int Shape::ReFormeCubicTo(int bord, livarot::Path *dest, livarot::Path *from, bool never_split)
{
  int nPiece = ebData[bord].pieceID;
  int nPath = ebData[bord].pathID;
  double ts = ebData[bord].tSt, te = ebData[bord].tEn;
  ...
  Geom::Point sDx, eDx;
  {
    ...
    livarot::Path::CubicTangent (ts, sDx, prevx,nData->start,nData->p,nData->end);
    livarot::Path::CubicTangent (te, eDx, prevx,nData->start,nData->p,nData->end);
  }
  sDx *= (te - ts);
  eDx *= (te - ts);
  {
