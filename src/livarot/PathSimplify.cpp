...
void livarot::Path::Simplify(double treshhold)
{
...
void livarot::Path::DoSimplify(int off, int N, double treshhold)
{
...
bool livarot::Path::FitCubic(Geom::Point const &start, PathDescrCubicTo &res,
...
bool livarot::Path::ExtendFit(int off, int N, fitting_tables &data, double treshhold, PathDescrCubicTo &res, int &worstP)
{
...
bool livarot::Path::AttemptSimplify (fitting_tables &data,double treshhold, PathDescrCubicTo & res,int &worstP)
{
...
bool livarot::Path::AttemptSimplify(int off, int N, double treshhold, PathDescrCubicTo &res,int &worstP)
{
    Geom::Point start;
    Geom::Point end;
...
double livarot::Path::RaffineTk (Geom::Point pt, Geom::Point p0, Geom::Point p1, Geom::Point p2, Geom::Point p3, double it)
{
...
void livarot::Path::Coalesce(double tresh)
{
    ...
    SetBackData(false);
    livarot::Path* tempDest = new livarot::Path();
...
void livarot::Path::FlushPendingAddition(livarot::Path *dest, PathDescr *lastAddition,
