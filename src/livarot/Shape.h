...
public:
     ...
     */
    void ConvertToForme(livarot::Path *dest);
     ...
     */
    void ConvertToForme(livarot::Path *dest, int nbP, livarot::Path *const *orig, bool never_split = false);
    // version trying to recover the nesting of subpaths (ie: holes)
    void ConvertToFormeNested(livarot::Path *dest, int nbP, livarot::Path *const *orig, int &nbNest, int *&nesting, int *&contStart, bool never_split = false);
...
private:
     ...
     */
    void AddContour(livarot::Path *dest, int num_orig, livarot::Path *const *orig, int start_edge, bool never_split = false);

    int ReFormeLineTo(int bord, livarot::Path *dest, bool never_split);
    int ReFormeArcTo(int bord, livarot::Path *dest, livarot::Path *orig, bool never_split);
    int ReFormeCubicTo(int bord, livarot::Path *dest, livarot::Path *orig, bool never_split);
