                          ...
                          void *data=nullptr)
{
    ...
    if ( collect == Inkscape::GC::AUTO ) {
        if ( scan == Inkscape::GC::SCANNED ) {
            mem = Inkscape::GC::Core::malloc(size);
        } else {
            mem = Inkscape::GC::Core::malloc_atomic(size);
        }
    } else {
        if ( scan == Inkscape::GC::SCANNED ) {
            mem = Inkscape::GC::Core::malloc_uncollectable(size);
        } else {
            mem = Inkscape::GC::Core::malloc_atomic_uncollectable(size);
        }
    }
    if (!mem) {
        throw std::bad_alloc();
    ...
    if (cleanup) {
        Inkscape::GC::Core::register_finalizer_ignore_self(mem, cleanup, data, nullptr, nullptr);
    }
    return mem;
}
