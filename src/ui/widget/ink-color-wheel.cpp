...
ColorWheel::ColorWheel(Inkscape::Colors::Space::Type type, std::vector<double> initial_color)
    ...
    for (int i = 0; i < height; ++i) {
        ...
        for (int j = 0; j < width; ++j) {
            ...
            if (r2 < r2_min || r2 > r2_max) {
                ...
                *p++ = Color(Inkscape::Colors::Space::Type::HSV, {hue, 1.0, 1.0}).toARGB();
            }
        }
    }
...
void ColorWheelHSL::on_drawing_area_draw(Cairo::RefPtr<Cairo::Context> const &cr, int, int)
{
    ...
    cr->restore();
    // Paint line on ring
    auto color_on_ring = Color(Inkscape::Colors::Space::Type::HSV, {_values[0], 1.0, 1.0});
...
bool ColorWheelHSL::_set_from_xy(double const x, double const y)
{
    ...
    ColorPoint c0(0, 0, Color(Inkscape::Colors::Space::Type::RGB, {yt, yt, yt}));      // Grey point along base.
    ColorPoint c1(0, 0, Color(Inkscape::Colors::Space::Type::HSV, {_values[0], 1, 1})); // Hue point at apex
...
std::array<ColorPoint, 3> const &ColorWheelHSL::get_triangle_corners()
{
    ...
    p0 = {x0, y0, Color(Inkscape::Colors::Space::Type::HSV, {_values[0], 1.0, 1.0})};
    p1 = {x1, y1, Color(Inkscape::Colors::Space::Type::HSV, {_values[0], 1.0, 0.0})};
    p2 = {x2, y2, Color(Inkscape::Colors::Space::Type::HSV, {_values[0], 0.0, 1.0})};
...
ColorWheelHSL::ColorWheelHSL()
    ...
    , ColorWheel(Inkscape::Colors::Space::Type::HSV, {0, 0, 0, 1})
{}

/* HSLuv Color Wheel */

ColorWheelHSLuv::ColorWheelHSLuv()
    : ColorWheel(Inkscape::Colors::Space::Type::HSLUV, {0, 1, 0.5, 1})
{
...
{
    if (_values.set(color, true)) {
        assert(_values.getSpace()->getType() == Inkscape::Colors::Space::Type::HSLUV);
        updateGeometry();
...
void ColorWheelHSLuv::on_drawing_area_draw(::Cairo::RefPtr<::Cairo::Context> const &cr, int, int)
{
    ...
    // Draw marker
    auto luv = Luv::toCoordinates(_values.converted(Inkscape::Colors::Space::Type::LUV)->getValues());
...
bool ColorWheelHSLuv::_set_from_xy(double const x, double const y)
{
    ...
    if (_values.set(Color(Inkscape::Colors::Space::Type::LUV, Luv::fromCoordinates({_values[2] * 100, p[Geom::X], p[Geom::Y]})), true)) {
        color_changed();
        return true;
    }
    return false;
...
void ColorWheelHSLuv::_updatePolygon()
{
    ...
    for (int y = bounding_min[Geom::Y]; y < bounding_max[Geom::Y]; y++) {
        for (int x = bounding_min[Geom::X]; x < bounding_max[Geom::X]; x++) {
            ...
            auto color = Color(Inkscape::Colors::Space::Type::LUV, Luv::fromCoordinates(color_vals));
                                     ...
                                     GdkModifierType const state)
{
    ...
    // Get current point
    auto luv = *_values.converted(Inkscape::Colors::Space::Type::LUV);
        ...
        double t)
{
    ...
    auto r0 = *v0.color.converted(Inkscape::Colors::Space::Type::RGB);
    auto r1 = *v1.color.converted(Inkscape::Colors::Space::Type::RGB);
    double r = lerp(r0[0], r1[0], t0, t1, t);
    double g = lerp(r0[1], r1[1], t0, t1, t);
    double b = lerp(r0[2], r1[2], t0, t1, t);

    return ColorPoint(x, y, Color(Inkscape::Colors::Space::Type::RGB, {r, g, b}));
}

// N.B. We also have Color:get_perceptual_lightness(), but that uses different weightings..!
double luminance(Color const &color)
{
    auto c = *color.converted(Inkscape::Colors::Space::Type::RGB);
