...
template <>
GValue from_bytes<Inkscape::Colors::Paint>(Glib::RefPtr<Glib::Bytes> &&bytes, char const *mime_type)
{
    try {
        return make_value(Inkscape::Colors::fromMIMEData(get_span(bytes), mime_type));
...
std::vector<GType> const &get_drop_types()
{
    static auto const instance = [] () -> std::vector<GType> {
        ...
        return {
            Glib::Value<Inkscape::Colors::Paint>::value_type(),
...
bool on_drop(Glib::ValueBase const &value, double x, double y, SPDesktopWidget *dtw, Glib::RefPtr<Gtk::DropTarget> const &drop_target)
{
    ...
    if (auto const ptr = get<Inkscape::Colors::Paint>(value)) {
        auto paint = *ptr;
