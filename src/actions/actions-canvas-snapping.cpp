...
struct SnapInfo {
    ...
    Inkscape::SnapTargetType type;       // corresponding snapping type
...
typedef std::unordered_map<Inkscape::SnapTargetType, Glib::ustring> SnapMap;

SnapVector snap_bbox = {
    { "snap-bbox",               Inkscape::SNAPTARGET_BBOX_CATEGORY,      true },
    { "snap-bbox-edge",          Inkscape::SNAPTARGET_BBOX_EDGE,          true },
    { "snap-bbox-corner",        Inkscape::SNAPTARGET_BBOX_CORNER,        true },
    { "snap-bbox-edge-midpoint", Inkscape::SNAPTARGET_BBOX_EDGE_MIDPOINT, false },
    { "snap-bbox-center",        Inkscape::SNAPTARGET_BBOX_MIDPOINT,      false },
};

SnapVector snap_node = {
    { "snap-node-category",      Inkscape::SNAPTARGET_NODE_CATEGORY,      true },
    { "snap-path",               Inkscape::SNAPTARGET_PATH,               true },
    { "snap-path-intersection",  Inkscape::SNAPTARGET_PATH_INTERSECTION,  false }, // Note: OFF by default, as it is extremely slow in large documents!
    { "snap-node-cusp",          Inkscape::SNAPTARGET_NODE_CUSP,          true },
    { "snap-node-smooth",        Inkscape::SNAPTARGET_NODE_SMOOTH,        true },
    { "snap-line-midpoint",      Inkscape::SNAPTARGET_LINE_MIDPOINT,      true },
    { "snap-line-tangential",    Inkscape::SNAPTARGET_PATH_TANGENTIAL,    false },
    { "snap-line-perpendicular", Inkscape::SNAPTARGET_PATH_PERPENDICULAR, false },
};

SnapVector snap_alignment = {
    { "snap-alignment",          Inkscape::SNAPTARGET_ALIGNMENT_CATEGORY,    true },
    { "snap-alignment-self",     Inkscape::SNAPTARGET_ALIGNMENT_HANDLE,      false },
    // separate category:
    { "snap-distribution",       Inkscape::SNAPTARGET_DISTRIBUTION_CATEGORY, true },
};

SnapVector snap_all_the_rest = {
    { "snap-others",             Inkscape::SNAPTARGET_OTHERS_CATEGORY,    true },
    { "snap-object-midpoint",    Inkscape::SNAPTARGET_OBJECT_MIDPOINT,    false },
    { "snap-rotation-center",    Inkscape::SNAPTARGET_ROTATION_CENTER,    false },
    { "snap-text-baseline",      Inkscape::SNAPTARGET_TEXT_BASELINE,      true },
    { "snap-path-mask",          Inkscape::SNAPTARGET_PATH_MASK,          true },
    { "snap-path-clip",          Inkscape::SNAPTARGET_PATH_CLIP,          true },

    { "snap-page-border",        Inkscape::SNAPTARGET_PAGE_EDGE_BORDER,   true },
    { "snap-page-margin",        Inkscape::SNAPTARGET_PAGE_MARGIN_BORDER, true },
    { "snap-grid",               Inkscape::SNAPTARGET_GRID,               true },
    { "snap-grid-line",          Inkscape::SNAPTARGET_GRID_LINE,          true },
    { "snap-guide",              Inkscape::SNAPTARGET_GUIDE,              true },
};

const struct {const char* action_name; Inkscape::SimpleSnap option; bool set;} simple_snap_options[] = {
    { "simple-snap-bbox",      Inkscape::SimpleSnap::BBox,      true },
    { "simple-snap-nodes",     Inkscape::SimpleSnap::Nodes,     true },
    { "simple-snap-alignment", Inkscape::SimpleSnap::Alignment, false }
};
...
//
void set_canvas_snapping(Inkscape::SnapTargetType type, bool enabled) {
...
static void canvas_snapping_toggle(Gio::ActionMap& map, Inkscape::SnapTargetType type) {
    bool enabled = get_snapping_preferences().isSnapButtonEnabled(type);
    set_canvas_snapping(type, !enabled);
    update_actions(map);
}

void set_simple_snap(Inkscape::SimpleSnap option, bool value) {
    const SnapVector* vect = nullptr;
    switch (option) {
    case Inkscape::SimpleSnap::BBox:
        vect = &snap_bbox;
        break;
    case Inkscape::SimpleSnap::Nodes:
        vect = &snap_node;
        break;
    case Inkscape::SimpleSnap::Alignment:
        vect = &snap_alignment;
        break;
    case Inkscape::SimpleSnap::Rest:
...
void toggle_simple_snap_option(Gio::ActionMap& map, Inkscape::SimpleSnap option) {
...
void apply_simple_snap_defaults(Gio::ActionMap& map) {
    set_simple_snap(Inkscape::SimpleSnap::BBox, true);
    set_simple_snap(Inkscape::SimpleSnap::Nodes, true);
    set_simple_snap(Inkscape::SimpleSnap::Alignment, false);
    set_simple_snap(Inkscape::SimpleSnap::Rest, true);
    update_actions(map);
}
...
void set_actions_canvas_snapping(Gio::ActionMap& map) {
    ...
    bool grid = snapprefs.isTargetSnappable(Inkscape::SNAPTARGET_GRID);
    ...
    struct { const char* action; bool state; bool enabled; } snap_options[] = {
        ...
        { "snap-path-clip",          snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_PATH_CLIP),          global && other},
        { "snap-path-mask",          snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_PATH_MASK),          global && other},

        { "snap-page-border",        snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_PAGE_EDGE_BORDER),   global },
        { "snap-page-margin",        snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_PAGE_MARGIN_BORDER), global },
        { "snap-grid",               snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_GRID),               global },
        { "snap-grid-line",          snapprefs.isSnapButtonEnabled(Inkscape::SNAPTARGET_GRID_LINE),          global && grid},
