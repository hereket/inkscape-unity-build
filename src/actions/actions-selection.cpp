 ...
 */
void
g_select_all(Glib::ustring condition, InkscapeApplication* app)
{
...
void 
g_select_invert(Glib::ustring condition, InkscapeApplication* app)
{
    if (condition != "" && condition != "layers" && condition != "no-layers" &&
        condition != "groups" && condition != "no-groups" && condition != "all") {
        show_output( "select_all: allowed options are '', 'all', 'layers', 'no-layers', 'groups', and 'no-groups'" );
...
add_actions_selection(InkscapeApplication* app)
{
    ...
    gapp->add_action_radio_string(  "select-all",                   sigc::bind(sigc::ptr_fun(&g_select_all),               app), "null");
    gapp->add_action_radio_string(  "select-invert",                sigc::bind(sigc::ptr_fun(&g_select_invert),            app), "null");
