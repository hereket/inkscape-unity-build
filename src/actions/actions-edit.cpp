...
add_actions_edit(InkscapeApplication* app)
{
    ...
    // clang-format off
    gapp->add_action( "object-to-pattern",               sigc::bind(sigc::ptr_fun(&ActionsEdit::object_to_pattern), app));
    gapp->add_action( "pattern-to-object",               sigc::bind(sigc::ptr_fun(&ActionsEdit::pattern_to_object), app));
    gapp->add_action( "object-to-marker",                sigc::bind(sigc::ptr_fun(&ActionsEdit::object_to_marker), app));
    gapp->add_action( "object-to-guides",                sigc::bind(sigc::ptr_fun(&ActionsEdit::object_to_guides), app));
    gapp->add_action( "cut",                             sigc::bind(sigc::ptr_fun(&ActionsEdit::cut), app));
    gapp->add_action( "copy",                            sigc::bind(sigc::ptr_fun(&ActionsEdit::copy), app));
    gapp->add_action( "paste-style",                     sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_style), app));
    gapp->add_action( "paste-size",                      sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_size), app));
    gapp->add_action( "paste-width",                     sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_width), app));
    gapp->add_action( "paste-height",                    sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_height), app));
    gapp->add_action( "paste-size-separately",           sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_size_separately), app));
    gapp->add_action( "paste-width-separately",          sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_width_separately), app));
    gapp->add_action( "paste-height-separately",         sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_height_separately), app));
    gapp->add_action( "duplicate",                       sigc::bind(sigc::ptr_fun(&ActionsEdit::duplicate), app));
    gapp->add_action( "duplicate-transform",             sigc::bind(sigc::ptr_fun(&ActionsEdit::duplicate_transform), app));
    ...
    gapp->add_action( "clone-unlink",                    sigc::bind(sigc::ptr_fun(&ActionsEdit::clone_unlink), app));
    gapp->add_action( "clone-unlink-recursively",        sigc::bind(sigc::ptr_fun(&ActionsEdit::clone_unlink_recursively), app));
    gapp->add_action( "clone-link",                      sigc::bind(sigc::ptr_fun(&ActionsEdit::clone_link), app));
    gapp->add_action( "select-original",                 sigc::bind(sigc::ptr_fun(&ActionsEdit::select_original), app));
    gapp->add_action( "clone-link-lpe",                  sigc::bind(sigc::ptr_fun(&ActionsEdit::clone_link_lpe), app));
    gapp->add_action( "delete",                          sigc::bind(sigc::ptr_fun(&ActionsEdit::edit_delete), app));
    gapp->add_action( "delete-selection",                sigc::bind(sigc::ptr_fun(&ActionsEdit::edit_delete_selection), app));
    gapp->add_action( "paste-path-effect",               sigc::bind(sigc::ptr_fun(&ActionsEdit::paste_path_effect), app));
    gapp->add_action( "remove-path-effect",              sigc::bind(sigc::ptr_fun(&ActionsEdit::remove_path_effect), app));
    gapp->add_action( "swap-fill-and-stroke",            sigc::bind(sigc::ptr_fun(&ActionsEdit::swap_fill_and_stroke), app));
    gapp->add_action( "fit-canvas-to-selection",         sigc::bind(sigc::ptr_fun(&ActionsEdit::fit_canvas_to_selection), app));
    // clang-format on
