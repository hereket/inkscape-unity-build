...
static Inkscape::XML::Node *sp_repr_svg_read_node (Document *xml_doc, xmlNodePtr node, const gchar *default_ns, std::map<std::string, std::string> &prefix_map);
...
static void sp_repr_write_stream_root_element(Inkscape::XML::Node *repr, Writer &out,
                                              bool add_whitespace, gchar const *default_ns,
                                              int inlineattrs, int indent,
                                              gchar const *old_href_abs_base,
                                              gchar const *new_href_abs_base);

static void sp_repr_write_stream_element(Inkscape::XML::Node *repr, Writer &out,
...
void promote_to_namespace(Inkscape::XML::Node *repr, const gchar *prefix) {
 ...
 */
void repair_namespace(Inkscape::XML::Node *repr, const gchar *prefix) {
    if ( repr->type() == Inkscape::XML::NodeType::ELEMENT_NODE ) {
...
Document *sp_repr_do_read (xmlDocPtr doc, const gchar *default_ns)
{
    ...
    Inkscape::XML::Node *root=nullptr;
    for ( node = doc->children ; node != nullptr ; node = node->next ) {
        if (node->type == XML_ELEMENT_NODE) {
            Inkscape::XML::Node *repr=sp_repr_svg_read_node(rdoc, node, default_ns, prefix_map);
...
static Inkscape::XML::Node *sp_repr_svg_read_node (Document *xml_doc, xmlNodePtr node, const gchar *default_ns, std::map<std::string, std::string> &prefix_map)
{
    ...
    Inkscape::XML::Node *repr = xml_doc->createElement(c);
    ...
    for (child = node->xmlChildrenNode; child != nullptr; child = child->next) {
        Inkscape::XML::Node *crepr = sp_repr_svg_read_node (xml_doc, child, default_ns, prefix_map);
        if (crepr) {
                    ...
                    gchar const *new_href_abs_base)
{
    ...
    const gchar *str = static_cast<Inkscape::XML::Node *>(doc)->attribute("doctype");
    if (str) {
        out->writeString( str );
    }

    for (Inkscape::XML::Node *repr = sp_repr_document_first_child(doc);
         repr; repr = repr->next())
    {
...
void populate_ns_map(NSMap &ns_map, Inkscape::XML::Node &repr) {
    if ( repr.type() == Inkscape::XML::NodeType::ELEMENT_NODE ) {
        ...
        for ( Inkscape::XML::Node *child=repr.firstChild() ;
...
void sp_repr_write_stream( Inkscape::XML::Node *repr, Writer &out, gint indent_level,
...
void sp_repr_write_stream_element( Inkscape::XML::Node * repr, Writer & out,
                                   gint indent_level, bool add_whitespace,
                                   Glib::QueryQuark elide_prefix,
                                   const AttributeVector & attributes, 
                                   int inlineattrs, int indent,
