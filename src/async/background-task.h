...
class BackgroundTask final {
    ...
    enum class TaskStatus { start, complete, cancelled, exception };
...
public:
    ...
    void cancel() {
        ...
        notify(TaskStatus::cancelled, std::optional<R>());
    }
...
private:
    ...
    public:
        Task(Async::Channel::Source source,
            ...
            duration throttle,
            std::function<void (TaskStatus, std::optional<R>)> notify) :
        ...
        duration _throttle;
        std::function<void (TaskStatus, std::optional<R>)> _notify;
    };
    ...
    void run(std::function<R (Progress<T...>&)> work, std::function<void (T...)> progress, duration throttle) {
        ...
        Task task(std::move(src), std::move(work), std::move(progress), throttle,
            [this](TaskStatus s, std::optional<R> result){ notify(s, result); });
    ...
    void notify(TaskStatus status, std::optional<R> result) {
