...
typedef std::set<PointPair > VisibilityVertSet;
...
static void vertexSweep(VertInf *vert)
{
    ...
    SweepEdgeList e;
    VisibilityVertSet::const_iterator vbegin = v.begin();
    VisibilityVertSet::const_iterator vend = v.end();
    const Point xaxis(DBL_MAX, centerInf->point.y);
    for (VisibilityVertSet::const_iterator t = vbegin; t != vend; ++t)
    {
        VertInf *k = t->vInf;
