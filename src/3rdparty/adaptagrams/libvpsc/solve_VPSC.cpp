...
static const double S_ZERO_UPPERBOUND=-1e-10;
...
bool Solver::satisfy() {
    ...
    for(unsigned i=0;i<m;i++) {
        ...
        if(cs[i]->slack() < S_ZERO_UPPERBOUND) {
#ifdef LIBVPSC_LOGGING
            ofstream f(LOGFILE,ios::app);
...
void Solver::refine() {
    ...
    for(unsigned i=0;i<m;i++) {
        if(cs[i]->slack() < S_ZERO_UPPERBOUND) {
            COLA_ASSERT(cs[i]->slack()>S_ZERO_UPPERBOUND);
            throw UnsatisfiedConstraint(*cs[i]);
        }
    }
}
...
bool IncSolver::satisfy() {
    ...
    while ( (v = mostViolated(inactive)) && 
            (v->equality || ((v->slack() < S_ZERO_UPPERBOUND) && !v->active)) ) 
    {
...
Constraint* IncSolver::mostViolated(Constraints &l)
{
    ...
    if ( (deleteIndex < lSize) && 
         (((slackForMostViolated < S_ZERO_UPPERBOUND) && !mostViolated->active) || 
