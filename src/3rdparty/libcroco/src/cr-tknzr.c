...
#define TKNZR_PRIVATE(obj) ((obj)->priv)
 ...
 *char peeked.
 */
#define TKNZR_PEEK_NEXT_CHAR(a_tknzr, a_to_char) \
{\
 ...
 *the character read.
 */
#define TKNZR_READ_NEXT_CHAR(a_tknzr, to_char) \
 ...
 */
#define TKNZR_RECORD_INITIAL_POS(a_tknzr, a_pos) \
status = cr_input_get_cur_pos (TKNZR_PRIVATE  \
(a_tknzr)->input, a_pos) ; \
 ...
 */
#define TKNZR_RECORD_CUR_BYTE_ADDR(a_tknzr, a_addr) \
status = cr_input_get_cur_byte_addr \
            (TKNZR_PRIVATE (a_tknzr)->input, a_addr) ; \
CHECK_PARSING_STATUS (status, TRUE)
 ...
 */
#define TKNZR_PEEK_BYTE(a_tknzr, a_offset, a_byte_ptr) \
status = cr_tknzr_peek_byte (a_tknzr, \
                             a_offset, \
                             a_byte_ptr) ; \
CHECK_PARSING_STATUS (status, TRUE) ;

#define TKNZR_BYTE(a_input, a_n, a_eof) \
 ...
 */
#define TKNZR_READ_NEXT_BYTE(a_tknzr, a_byte_ptr) \
status = \
cr_input_read_byte (TKNZR_PRIVATE (a_tknzr)->input, a_byte_ptr) ;\
CHECK_PARSING_STATUS (status, TRUE) ;
 ...
 */
#define TKNZR_SKIP_BYTES(a_tknzr, a_nb_bytes) \
status = cr_input_seek_index (TKNZR_PRIVATE (a_tknzr)->input, \
 ...
 *type glong.
 */
#define TKZNR_SKIP_CHARS(a_tknzr, a_nb_chars) \
{ \
gulong nb_chars = a_nb_chars ; \
status = cr_input_consume_chars \
     (TKNZR_PRIVATE (a_tknzr)->input,0, &nb_chars) ; \
CHECK_PARSING_STATUS (status, TRUE) ; \
}
                  ...
                  CRParsingLocation *a_location)
{
        ...
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this)
                              && TKNZR_PRIVATE (a_this)->input
                              && a_start && a_end, 
                              CR_BAD_PARAM_ERROR);

        TKNZR_RECORD_INITIAL_POS (a_this, &init_pos);

        *a_start = NULL;
        *a_end = NULL;

        TKNZR_READ_NEXT_CHAR (a_this, &cur_char);
        ...
        TKNZR_RECORD_CUR_BYTE_ADDR (a_this, a_start);
        *a_end = *a_start;

        for (;;) {
                gboolean is_eof = FALSE;

                cr_input_get_end_of_file (TKNZR_PRIVATE (a_this)->input, &is_eof);
                if (is_eof)
                        break;
                ...
                if (cr_utils_is_white_space (cur_char) == TRUE) {
                        TKNZR_READ_NEXT_CHAR (a_this, &cur_char);
                        TKNZR_RECORD_CUR_BYTE_ADDR (a_this, a_end);
                } else {
                        break;
                }
        }
                   ...
                   CRParsingLocation *a_location)
{
        ...
        TKNZR_PEEK_BYTE (a_this, 1, &next_chars[0]);
        TKNZR_PEEK_BYTE (a_this, 2, &next_chars[1]);

        if ((next_chars[0] == '\r' && next_chars[1] == '\n')) {
                TKNZR_SKIP_BYTES (a_this, 1);
                if (a_location) {
                        cr_tknzr_get_parsing_location 
                                (a_this, a_location) ;
                }
                TKZNR_SKIP_CHARS (a_this, 1);
...
cr_tknzr_try_to_skip_spaces (CRTknzr * a_this)
{
        ...
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this)
                              && TKNZR_PRIVATE (a_this)->input, CR_BAD_PARAM_ERROR);

        status = cr_input_peek_char (TKNZR_PRIVATE (a_this)->input, &cur_char);
        ...
        if (cr_utils_is_white_space (cur_char) == TRUE) {
                ...
                status = cr_input_consume_white_spaces
                        (TKNZR_PRIVATE (a_this)->input, &nb_chars);
        }
                        ...
                        CRString ** a_comment)
{
        ...
        for (;;) { /* [^*]* */
                TKNZR_PEEK_NEXT_CHAR (a_this, &next_char);
                if (next_char == '*')
                        break;
                TKNZR_READ_NEXT_CHAR (a_this, &cur_char);
                               ...
                               CRParsingLocation *a_location)
{
        ...
        TKNZR_PEEK_NEXT_CHAR (a_this, &cur_char);
                       ...
                       CRParsingLocation *a_location)
{
        ...
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this) && a_char,
                              CR_BAD_PARAM_ERROR);

        TKNZR_RECORD_INITIAL_POS (a_this, &init_pos);

        status = cr_input_peek_char (TKNZR_PRIVATE (a_this)->input, 
                                     &next_char) ;
        if (status != CR_OK)
                goto error;
                    ...
                    CRString ** a_str)
{
        ...
        TKNZR_PEEK_BYTE (a_this, 1, &tab[0]);
        TKNZR_PEEK_BYTE (a_this, 2, &tab[1]);
        TKNZR_PEEK_BYTE (a_this, 3, &tab[2]);
        TKNZR_PEEK_BYTE (a_this, 4, &tab[3]);
...
cr_tknzr_parse_rgb (CRTknzr * a_this, CRRgb ** a_rgb)
{
        ...
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this), CR_BAD_PARAM_ERROR);

        TKNZR_RECORD_INITIAL_POS (a_this, &init_pos);

        TKNZR_PEEK_BYTE (a_this, 1, &next_bytes[0]);
        TKNZR_PEEK_BYTE (a_this, 2, &next_bytes[1]);
        TKNZR_PEEK_BYTE (a_this, 3, &next_bytes[2]);
        ...
        if (((next_bytes[0] == 'r') || (next_bytes[0] == 'R'))
            ...
            && ((next_bytes[2] == 'b') || (next_bytes[2] == 'B'))) {
                TKZNR_SKIP_CHARS (a_this, 1);
                cr_tknzr_get_parsing_location (a_this, &location) ;
                TKZNR_SKIP_CHARS (a_this, 2);
        } else {
                status = CR_PARSING_ERROR;
                goto error;
        }
        TKNZR_READ_NEXT_BYTE (a_this, &cur_byte);
                          ...
                          CRParsingLocation *a_location)
{
        ...
        if (TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 1, NULL) == 'i'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 2, NULL) == 'm'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 3, NULL) == 'p'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 4, NULL) == 'o'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 5, NULL) == 'r'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 6, NULL) == 't'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 7, NULL) == 'a'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 8, NULL) == 'n'
            && TKNZR_BYTE (TKNZR_PRIVATE (a_this)->input, 9, NULL) == 't') {
                TKNZR_SKIP_BYTES (a_this, 9);
                if (a_location) {
                        cr_tknzr_get_parsing_location (a_this,
                                                       a_location) ;
                }
                    ...
                    CRNum ** a_num)
{
        ...
        for (;;) {
                ...
                if (next_char == '.') {
                        ...
                        guchar next_byte;
                        TKNZR_PEEK_BYTE (a_this, 2, &next_byte)
                        if (next_byte == '+') {
                                TKNZR_READ_NEXT_CHAR (a_this, &cur_char);
                        } else if (next_byte == '-') {
                                exp_sign = -1;
...
{
        g_return_if_fail (a_this && TKNZR_PRIVATE (a_this));

        TKNZR_PRIVATE (a_this)->ref_count++;
}

gboolean
cr_tknzr_unref (CRTknzr * a_this)
{
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this), FALSE);

        if (TKNZR_PRIVATE (a_this)->ref_count > 0) {
                TKNZR_PRIVATE (a_this)->ref_count--;
        }
...
cr_tknzr_set_input (CRTknzr * a_this, CRInput * a_input)
{
        ...
        if (TKNZR_PRIVATE (a_this)->input) {
                cr_input_unref (TKNZR_PRIVATE (a_this)->input);
        }

        TKNZR_PRIVATE (a_this)->input = a_input;

        cr_input_ref (TKNZR_PRIVATE (a_this)->input);
...
cr_tknzr_get_input (CRTknzr * a_this, CRInput ** a_input)
{
        ...
        *a_input = TKNZR_PRIVATE (a_this)->input;
...
cr_tknzr_read_byte (CRTknzr * a_this, guchar * a_byte)
{
        ...
        return cr_input_read_byte (TKNZR_PRIVATE (a_this)->input, a_byte);
...
cr_tknzr_read_char (CRTknzr * a_this, guint32 * a_char)
{
        ...
        if (TKNZR_PRIVATE (a_this)->token_cache) {
                cr_input_set_cur_pos (TKNZR_PRIVATE (a_this)->input,
                                      &TKNZR_PRIVATE (a_this)->prev_pos);
                cr_token_destroy (TKNZR_PRIVATE (a_this)->token_cache);
                TKNZR_PRIVATE (a_this)->token_cache = NULL;
        }

        return cr_input_read_char (TKNZR_PRIVATE (a_this)->input, a_char);
}
...
cr_tknzr_peek_char (CRTknzr * a_this, guint32 * a_char)
{
        ...
        return cr_input_peek_char (TKNZR_PRIVATE (a_this)->input, a_char);
}
...
{
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this)
                              && TKNZR_PRIVATE (a_this)->input && a_byte,
                              CR_BAD_PARAM_ERROR);
        ...
        return cr_input_peek_byte (TKNZR_PRIVATE (a_this)->input,
...
{
        g_return_val_if_fail (a_this && TKNZR_PRIVATE (a_this)
                              && TKNZR_PRIVATE (a_this)->input, 0);

        return cr_input_peek_byte2 (TKNZR_PRIVATE (a_this)->input, a_offset, a_eof);
}
...
cr_tknzr_get_nb_bytes_left (CRTknzr * a_this)
{
        ...
        return cr_input_get_nb_bytes_left (TKNZR_PRIVATE (a_this)->input);
}
...
cr_tknzr_get_cur_pos (CRTknzr * a_this, CRInputPos * a_pos)
{
        ...
        return cr_input_get_cur_pos (TKNZR_PRIVATE (a_this)->input, a_pos);
}
                               ...
                               CRParsingLocation *a_loc)
{
        ...
        return cr_input_get_parsing_location 
                (TKNZR_PRIVATE (a_this)->input, a_loc) ;
}
...
cr_tknzr_get_cur_byte_addr (CRTknzr * a_this, guchar ** a_addr)
{
        ...
        return cr_input_get_cur_byte_addr (TKNZR_PRIVATE (a_this)->input, a_addr);
}
...
cr_tknzr_seek_index (CRTknzr * a_this, enum CRSeekPos a_origin, gint a_pos)
{
        ...
        return cr_input_seek_index (TKNZR_PRIVATE (a_this)->input, a_origin, a_pos);
}
...
cr_tknzr_consume_chars (CRTknzr * a_this, guint32 a_char, glong * a_nb_char)
{
	...
	enum CRStatus status;
        ...
        status = cr_input_consume_chars (TKNZR_PRIVATE (a_this)->input,
                                         a_char, &consumed);
...
cr_tknzr_set_cur_pos (CRTknzr * a_this, CRInputPos * a_pos)
{
        ...
        return cr_input_set_cur_pos (TKNZR_PRIVATE (a_this)->input, a_pos);
}
...
cr_tknzr_unget_token (CRTknzr * a_this, CRToken * a_token)
{
        ...
        TKNZR_PRIVATE (a_this)->token_cache = a_token;
...
cr_tknzr_get_next_token (CRTknzr * a_this, CRToken ** a_tk)
{
        ...
        if (TKNZR_PRIVATE (a_this)->token_cache) {
                *a_tk = TKNZR_PRIVATE (a_this)->token_cache;
                TKNZR_PRIVATE (a_this)->token_cache = NULL;
                return CR_OK;
        }
        ...
        status = cr_input_get_end_of_file
                (TKNZR_PRIVATE (a_this)->input, &reached_eof);
        ...
        input = TKNZR_PRIVATE (a_this)->input;
        ...
        case '@':
                {
                        if (TKNZR_BYTE (input, 2, NULL) == 'f'
                            && TKNZR_BYTE (input, 3, NULL) == 'o'
                            && TKNZR_BYTE (input, 4, NULL) == 'n'
                            && TKNZR_BYTE (input, 5, NULL) == 't'
                            && TKNZR_BYTE (input, 6, NULL) == '-'
                            && TKNZR_BYTE (input, 7, NULL) == 'f'
                            && TKNZR_BYTE (input, 8, NULL) == 'a'
                            && TKNZR_BYTE (input, 9, NULL) == 'c'
                            && TKNZR_BYTE (input, 10, NULL) == 'e') {
                                TKZNR_SKIP_CHARS (a_this, 1);
                                cr_tknzr_get_parsing_location 
                                        (a_this, &location) ;
                                TKZNR_SKIP_CHARS (a_this, 9);
                        ...
                        if (TKNZR_BYTE (input, 2, NULL) == 'c'
                            && TKNZR_BYTE (input, 3, NULL) == 'h'
                            && TKNZR_BYTE (input, 4, NULL) == 'a'
                            && TKNZR_BYTE (input, 5, NULL) == 'r'
                            && TKNZR_BYTE (input, 6, NULL) == 's'
                            && TKNZR_BYTE (input, 7, NULL) == 'e'
                            && TKNZR_BYTE (input, 8, NULL) == 't') {
                                TKZNR_SKIP_CHARS (a_this, 1);
                                cr_tknzr_get_parsing_location
                                        (a_this, &location) ;
                                TKZNR_SKIP_CHARS (a_this, 7);
                        ...
                        if (TKNZR_BYTE (input, 2, NULL) == 'i'
                            && TKNZR_BYTE (input, 3, NULL) == 'm'
                            && TKNZR_BYTE (input, 4, NULL) == 'p'
                            && TKNZR_BYTE (input, 5, NULL) == 'o'
                            && TKNZR_BYTE (input, 6, NULL) == 'r'
                            && TKNZR_BYTE (input, 7, NULL) == 't') {
                                TKZNR_SKIP_CHARS (a_this, 1);
                                cr_tknzr_get_parsing_location 
                                        (a_this, &location) ;
                                TKZNR_SKIP_CHARS (a_this, 6);
                        ...
                        if (TKNZR_BYTE (input, 2, NULL) == 'm'
                            && TKNZR_BYTE (input, 3, NULL) == 'e'
                            && TKNZR_BYTE (input, 4, NULL) == 'd'
                            && TKNZR_BYTE (input, 5, NULL) == 'i'
                            && TKNZR_BYTE (input, 6, NULL) == 'a') {
                                TKZNR_SKIP_CHARS (a_this, 1);
                                cr_tknzr_get_parsing_location (a_this, 
                                                               &location) ;
                                TKZNR_SKIP_CHARS (a_this, 5);
                        ...
                        if (TKNZR_BYTE (input, 2, NULL) == 'p'
                            && TKNZR_BYTE (input, 3, NULL) == 'a'
                            && TKNZR_BYTE (input, 4, NULL) == 'g'
                            && TKNZR_BYTE (input, 5, NULL) == 'e') {
                                TKZNR_SKIP_CHARS (a_this, 1);
                                cr_tknzr_get_parsing_location (a_this, 
                                                               &location) ;
                                TKZNR_SKIP_CHARS (a_this, 4);
        ...
        case 'u':
                ...
                if (TKNZR_BYTE (input, 2, NULL) == 'r'
                    && TKNZR_BYTE (input, 3, NULL) == 'l'
                    && TKNZR_BYTE (input, 4, NULL) == '(') {
                        CRString *str2 = NULL;
        ...
        case 'r':
                if (TKNZR_BYTE (input, 2, NULL) == 'g'
                    && TKNZR_BYTE (input, 3, NULL) == 'b'
                    && TKNZR_BYTE (input, 4, NULL) == '(') {
        ...
        case '<':
                if (TKNZR_BYTE (input, 2, NULL) == '!'
                    && TKNZR_BYTE (input, 3, NULL) == '-'
                    && TKNZR_BYTE (input, 4, NULL) == '-') {
                        TKZNR_SKIP_CHARS (a_this, 1);
                        cr_tknzr_get_parsing_location (a_this, 
                                                       &location) ;
        ...
        parse_number:
                {
                        ...
                        status = cr_tknzr_parse_num (a_this, &num);
                        if (status == CR_OK && num) {
                                next_bytes[0] = TKNZR_BYTE (input, 1, NULL);
                                next_bytes[1] = TKNZR_BYTE (input, 2, NULL);
                                next_bytes[2] = TKNZR_BYTE (input, 3, NULL);
                                next_bytes[3] = TKNZR_BYTE (input, 4, NULL);
                    ...
                    || ((next_char >= 'A') && (next_char <= 'Z'))) {
                        ...
                        if (status == CR_OK && str) {
                                ...
                                status = cr_input_peek_char
                                        (TKNZR_PRIVATE (a_this)->input, &next_c);
        ...
        TKNZR_READ_NEXT_CHAR (a_this, &next_char);
        cr_tknzr_get_parsing_location (a_this, 
                                       &location) ;
        ...
        if (status == CR_OK && token) {
                *a_tk = token;
                 ...
                 */
                memmove (&TKNZR_PRIVATE (a_this)->prev_pos,
...
cr_tknzr_destroy (CRTknzr * a_this)
{
        ...
        if (TKNZR_PRIVATE (a_this) && TKNZR_PRIVATE (a_this)->input) {
                if (cr_input_unref (TKNZR_PRIVATE (a_this)->input)
                    == TRUE) {
                        TKNZR_PRIVATE (a_this)->input = NULL;
                }
        }
        ...
        if (TKNZR_PRIVATE (a_this)) {
                g_free (TKNZR_PRIVATE (a_this));
                TKNZR_PRIVATE (a_this) = NULL;
        }
