...
#define INPUT_PRIVATE(object) (object)->priv
...
cr_input_new_real (void)
{
        ...
        INPUT_PRIVATE (result) = g_try_malloc (sizeof (CRInputPriv));
        if (!INPUT_PRIVATE (result)) {
                cr_utils_trace_info ("Out of memory");
                g_free (result);
                return NULL;
        }
        memset (INPUT_PRIVATE (result), 0, sizeof (CRInputPriv));
        INPUT_PRIVATE (result)->free_in_buf = TRUE;
        return result;
}
        ...
        if (a_enc != CR_UTF_8) {
                ...
                status = cr_enc_handler_convert_input
                        (enc_handler, a_buf, &len,
                         &INPUT_PRIVATE (result)->in_buf,
                         &INPUT_PRIVATE (result)->in_buf_size);
                if (status != CR_OK)
                        goto error;
                INPUT_PRIVATE (result)->free_in_buf = TRUE;
                ...
                INPUT_PRIVATE (result)->nb_bytes = INPUT_PRIVATE (result)->in_buf_size;
        } else {
                INPUT_PRIVATE (result)->in_buf = (guchar *) a_buf;
                INPUT_PRIVATE (result)->in_buf_size = a_len;
                INPUT_PRIVATE (result)->nb_bytes = a_len;
                INPUT_PRIVATE (result)->free_in_buf = a_free_buf;
        }
        INPUT_PRIVATE (result)->line = 1;
        INPUT_PRIVATE (result)->col =  0;
        return result;
...
cr_input_destroy (CRInput * a_this)
{
        ...
        if (INPUT_PRIVATE (a_this)) {
                if (INPUT_PRIVATE (a_this)->in_buf && INPUT_PRIVATE (a_this)->free_in_buf) {
                        g_free (INPUT_PRIVATE (a_this)->in_buf);
                        INPUT_PRIVATE (a_this)->in_buf = NULL;
                }

                g_free (INPUT_PRIVATE (a_this));
                INPUT_PRIVATE (a_this) = NULL;
        }
...
{
        g_return_if_fail (a_this && INPUT_PRIVATE (a_this));

        INPUT_PRIVATE (a_this)->ref_count++;
}
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this), FALSE);

        if (INPUT_PRIVATE (a_this)->ref_count) {
                INPUT_PRIVATE (a_this)->ref_count--;
        }

        if (INPUT_PRIVATE (a_this)->ref_count == 0) {
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this)
                              && a_end_of_input, CR_BAD_PARAM_ERROR);

        *a_end_of_input = (INPUT_PRIVATE (a_this)->next_byte_index
                           >= INPUT_PRIVATE (a_this)->in_buf_size) ? TRUE : FALSE;
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this), -1);
        g_return_val_if_fail (INPUT_PRIVATE (a_this)->nb_bytes
                              <= INPUT_PRIVATE (a_this)->in_buf_size, -1);
        g_return_val_if_fail (INPUT_PRIVATE (a_this)->next_byte_index
                              <= INPUT_PRIVATE (a_this)->nb_bytes, -1);

        if (INPUT_PRIVATE (a_this)->end_of_input)
                return 0;

        return INPUT_PRIVATE (a_this)->nb_bytes - INPUT_PRIVATE (a_this)->next_byte_index;
}
...
cr_input_read_byte (CRInput * a_this, guchar * a_byte)
{
        ...
        g_return_val_if_fail (INPUT_PRIVATE (a_this)->next_byte_index <=
                              INPUT_PRIVATE (a_this)->nb_bytes, CR_BAD_PARAM_ERROR);

        if (INPUT_PRIVATE (a_this)->end_of_input == TRUE)
                return CR_END_OF_INPUT_ERROR;

        *a_byte = INPUT_PRIVATE (a_this)->in_buf[INPUT_PRIVATE (a_this)->next_byte_index];

        if (INPUT_PRIVATE (a_this)->nb_bytes -
            INPUT_PRIVATE (a_this)->next_byte_index < 2) {
                INPUT_PRIVATE (a_this)->end_of_input = TRUE;
        } else {
                INPUT_PRIVATE (a_this)->next_byte_index++;
        }
...
cr_input_read_char (CRInput * a_this, guint32 * a_char)
{
        ...
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_char,
                              CR_BAD_PARAM_ERROR);
        ...
        if (status == CR_OK) {
                ...
                INPUT_PRIVATE (a_this)->next_byte_index += consumed;

                /*update line and column number */
                if (INPUT_PRIVATE (a_this)->end_of_line == TRUE) {
                        INPUT_PRIVATE (a_this)->col = 1;
                        INPUT_PRIVATE (a_this)->line++;
                        INPUT_PRIVATE (a_this)->end_of_line = FALSE;
                } else if (*a_char != '\n') {
                        INPUT_PRIVATE (a_this)->col++;
                }

                if (*a_char == '\n') {
                        INPUT_PRIVATE (a_this)->end_of_line = TRUE;
                }
        }
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this), CR_BAD_PARAM_ERROR);

        INPUT_PRIVATE (a_this)->line = a_line_num;
...
cr_input_get_line_num (CRInput const * a_this, glong * a_line_num)
{
        ...
        *a_line_num = INPUT_PRIVATE (a_this)->line;
...
cr_input_set_column_num (CRInput * a_this, glong a_col)
{
        ...
        INPUT_PRIVATE (a_this)->col = a_col;
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_col,
                              CR_BAD_PARAM_ERROR);

        *a_col = INPUT_PRIVATE (a_this)->col;
...
cr_input_increment_line_num (CRInput * a_this, glong a_increment)
{
        ...
        INPUT_PRIVATE (a_this)->line += a_increment;
...
cr_input_increment_col_num (CRInput * a_this, glong a_increment)
{
        ...
        INPUT_PRIVATE (a_this)->col += a_increment;
...
cr_input_consume_chars (CRInput * a_this, guint32 a_char, gulong * a_nb_char)
{
        ...
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_nb_char,
                              CR_BAD_PARAM_ERROR);
...
cr_input_consume_white_spaces (CRInput * a_this, gulong * a_nb_chars)
{
        ...
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_nb_chars,
                              CR_BAD_PARAM_ERROR);
...
cr_input_peek_char (CRInput const * a_this, guint32 * a_char)
{
        ...
        if (INPUT_PRIVATE (a_this)->next_byte_index >=
            INPUT_PRIVATE (a_this)->in_buf_size) {
                return CR_END_OF_INPUT_ERROR;
        }
                    ...
                    gulong a_offset, guchar * a_byte)
{
        ...
        case CR_SEEK_CUR:
                abs_offset = INPUT_PRIVATE (a_this)->next_byte_index - 1 + a_offset;
                break;
        ...
        case CR_SEEK_END:
                abs_offset = INPUT_PRIVATE (a_this)->in_buf_size - 1 - a_offset;
                break;
        ...
        if (abs_offset < INPUT_PRIVATE (a_this)->in_buf_size) {

                *a_byte = INPUT_PRIVATE (a_this)->in_buf[abs_offset];
...
cr_input_peek_byte2 (CRInput const * a_this, gulong a_offset, gboolean * a_eof)
{
        ...
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this), 0);
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this), NULL);

        if (a_offset >= INPUT_PRIVATE (a_this)->nb_bytes) {
                return NULL;
        }

        return &INPUT_PRIVATE (a_this)->in_buf[a_offset];
}
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_offset,
                              CR_BAD_PARAM_ERROR);
        ...
        *a_offset = cr_input_get_byte_addr
                (a_this, INPUT_PRIVATE (a_this)->next_byte_index - 1);
...
cr_input_seek_index (CRInput * a_this, enum CRSeekPos a_origin, gint a_pos)
{
        ...
        case CR_SEEK_CUR:
                abs_offset = INPUT_PRIVATE (a_this)->next_byte_index - 1 + a_pos;
                break;
        ...
        case CR_SEEK_END:
                abs_offset = INPUT_PRIVATE (a_this)->in_buf_size - 1 - a_pos;
                break;
        ...
        if ((abs_offset > 0)
            && (gulong) abs_offset < INPUT_PRIVATE (a_this)->nb_bytes) {

                /*update the input stream's internal state */
                INPUT_PRIVATE (a_this)->next_byte_index = abs_offset + 1;
...
{
        g_return_val_if_fail (a_this && INPUT_PRIVATE (a_this) && a_pos,
                              CR_BAD_PARAM_ERROR);

        a_pos->next_byte_index = INPUT_PRIVATE (a_this)->next_byte_index;
        a_pos->line = INPUT_PRIVATE (a_this)->line;
        a_pos->col = INPUT_PRIVATE (a_this)->col;
        a_pos->end_of_line = INPUT_PRIVATE (a_this)->end_of_line;
        a_pos->end_of_file = INPUT_PRIVATE (a_this)->end_of_input;
                               ...
                               CRParsingLocation *a_loc)
{
        g_return_val_if_fail (a_this 
                              && INPUT_PRIVATE (a_this)
                              && a_loc, 
                              CR_BAD_PARAM_ERROR) ;

        a_loc->line = INPUT_PRIVATE (a_this)->line ;
        a_loc->column = INPUT_PRIVATE (a_this)->col ;
        if (INPUT_PRIVATE (a_this)->next_byte_index) {
                a_loc->byte_offset = INPUT_PRIVATE (a_this)->next_byte_index - 1 ;
        } else {
                a_loc->byte_offset = INPUT_PRIVATE (a_this)->next_byte_index  ;
        }
        return CR_OK ;
}
...
cr_input_get_cur_index (CRInput const * a_this, glong * a_index)
{
        ...
        *a_index = INPUT_PRIVATE (a_this)->next_byte_index;
...
cr_input_set_cur_index (CRInput * a_this, glong a_index)
{
        ...
        INPUT_PRIVATE (a_this)->next_byte_index = a_index;
...
cr_input_set_end_of_file (CRInput * a_this, gboolean a_eof)
{
        ...
        INPUT_PRIVATE (a_this)->end_of_input = a_eof;
...
cr_input_get_end_of_file (CRInput const * a_this, gboolean * a_eof)
{
        ...
        *a_eof = INPUT_PRIVATE (a_this)->end_of_input;
...
cr_input_set_end_of_line (CRInput * a_this, gboolean a_eol)
{
        ...
        INPUT_PRIVATE (a_this)->end_of_line = a_eol;
...
cr_input_get_end_of_line (CRInput const * a_this, gboolean * a_eol)
{
        ...
        *a_eol = INPUT_PRIVATE (a_this)->end_of_line;
