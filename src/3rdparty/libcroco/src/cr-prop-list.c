...
#define PROP_PRIVATE(a_obj) (a_obj)->priv
...
cr_prop_list_allocate (void)
{
        ...
        PROP_PRIVATE (result) = g_try_malloc (sizeof (CRPropListPriv));
        if (!PROP_PRIVATE (result)) {
                cr_utils_trace_info ("could not allocate CRPropListPriv");
                g_free (result);
                return NULL;
        }
        memset (PROP_PRIVATE (result), 0, sizeof (CRPropListPriv));
        return result;
}
...
cr_prop_list_append (CRPropList * a_this, CRPropList * a_to_append)
{
        ...
        for (cur = a_this;
             cur && PROP_PRIVATE (cur) && PROP_PRIVATE (cur)->next;
             cur = PROP_PRIVATE (cur)->next) ;
        g_return_val_if_fail (cur, NULL);
        PROP_PRIVATE (cur)->next = a_to_append;
        PROP_PRIVATE (a_to_append)->prev = cur;
        return a_this;
}
		      ...
		      CRDeclaration * a_decl)
{
        ...
        g_return_val_if_fail (list && PROP_PRIVATE (list), NULL);

        PROP_PRIVATE (list)->prop = a_prop;
        PROP_PRIVATE (list)->decl = a_decl;
...
cr_prop_list_prepend (CRPropList * a_this, CRPropList * a_to_prepend)
{
        ...
        for (cur = a_to_prepend; cur && PROP_PRIVATE (cur)->next;
             cur = PROP_PRIVATE (cur)->next) ;
        g_return_val_if_fail (cur, NULL);
        PROP_PRIVATE (cur)->next = a_this;
        PROP_PRIVATE (a_this)->prev = cur;
        return a_to_prepend;
}
                       ...
                       CRString * a_prop_name, CRDeclaration * a_decl)
{
        ...
        g_return_val_if_fail (a_this && PROP_PRIVATE (a_this)
                              && a_prop_name && a_decl, NULL);

        list = cr_prop_list_allocate ();
        g_return_val_if_fail (list, NULL);
        PROP_PRIVATE (list)->prop = a_prop_name;
        PROP_PRIVATE (list)->decl = a_decl;
...
cr_prop_list_set_prop (CRPropList * a_this, CRString * a_prop)
{
        ...
        PROP_PRIVATE (a_this)->prop = a_prop;
        return CR_OK;
}
...
cr_prop_list_get_prop (CRPropList const * a_this, CRString ** a_prop)
{
        ...
        *a_prop = PROP_PRIVATE (a_this)->prop;
        return CR_OK;
}
...
cr_prop_list_set_decl (CRPropList * a_this, CRDeclaration * a_decl)
{
        ...
        PROP_PRIVATE (a_this)->decl = a_decl;
        return CR_OK;
}
...
cr_prop_list_get_decl (CRPropList const * a_this, CRDeclaration ** a_decl)
{
        ...
        *a_decl = PROP_PRIVATE (a_this)->decl;
        return CR_OK;
}
                          ...
                          CRString * a_prop, CRPropList ** a_pair)
{
        ...
        g_return_val_if_fail (PROP_PRIVATE (a_this), CR_BAD_PARAM_ERROR);

        for (cur = a_this; cur; cur = PROP_PRIVATE (cur)->next) {
                if (PROP_PRIVATE (cur)->prop
		    && PROP_PRIVATE (cur)->prop->stryng
                    && PROP_PRIVATE (cur)->prop->stryng->str
		    && a_prop->stryng
                    && a_prop->stryng->str
                    && !strcmp (PROP_PRIVATE (cur)->prop->stryng->str, 
				a_prop->stryng->str))
                        break;
        }
...
{
        g_return_val_if_fail (a_this && PROP_PRIVATE (a_this), NULL);

        return PROP_PRIVATE (a_this)->next;
}
...
cr_prop_list_get_prev (CRPropList * a_this)
{
        ...
        return PROP_PRIVATE (a_this)->prev;
}
...
cr_prop_list_unlink (CRPropList * a_this, CRPropList * a_pair)
{
        ...
        g_return_val_if_fail (a_this && PROP_PRIVATE (a_this) && a_pair, NULL);

        /*some sanity checks */
        if (PROP_PRIVATE (a_pair)->next) {
                next = PROP_PRIVATE (a_pair)->next;
                g_return_val_if_fail (PROP_PRIVATE (next), NULL);
                g_return_val_if_fail (PROP_PRIVATE (next)->prev == a_pair, NULL);
        }
        if (PROP_PRIVATE (a_pair)->prev) {
                prev = PROP_PRIVATE (a_pair)->prev;
                g_return_val_if_fail (PROP_PRIVATE (prev), NULL);
                g_return_val_if_fail (PROP_PRIVATE (prev)->next == a_pair, NULL);
        }
        if (prev) {
                PROP_PRIVATE (prev)->next = next;
        }
        if (next) {
                PROP_PRIVATE (next)->prev = prev;
        }
        PROP_PRIVATE (a_pair)->prev = PROP_PRIVATE (a_pair)->next = NULL;
        if (a_this == a_pair) {
                if (next)
                        return next;
                return NULL;
...
cr_prop_list_destroy (CRPropList * a_this)
{
        ...
        g_return_if_fail (a_this && PROP_PRIVATE (a_this));

        for (tail = a_this;
             tail && PROP_PRIVATE (tail) && PROP_PRIVATE (tail)->next;
        ...
        while (cur) {
                tail = PROP_PRIVATE (cur)->prev;
                if (tail && PROP_PRIVATE (tail))
                        PROP_PRIVATE (tail)->next = NULL;
                PROP_PRIVATE (cur)->prev = NULL;
                g_free (PROP_PRIVATE (cur));
                PROP_PRIVATE (cur) = NULL;
                g_free (cur);
                cur = tail;
        }
}
