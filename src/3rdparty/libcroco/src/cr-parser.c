...
#define PARSER_PRIVATE(obj) ((obj)->priv)
...
{\
status = cr_tknzr_peek_char (PARSER_PRIVATE (a_this)->tknzr, a_to_char) ; \
...
status = cr_tknzr_read_char (PARSER_PRIVATE (a_this)->tknzr, a_to_char) ; \
CHECK_PARSING_STATUS (status, TRUE)
...
status = cr_tknzr_get_cur_pos (PARSER_PRIVATE \
(a_this)->tknzr, a_pos) ; \
...
status = cr_tknzr_get_cur_byte_addr \
            (PARSER_PRIVATE (a_this)->tknzr, a_addr) ; \
CHECK_PARSING_STATUS (status, TRUE)
...
status = cr_tknzr_peek_byte (PARSER_PRIVATE (a_this)->tknzr, \
                              a_offset, \
                              a_byte_ptr) ; \
CHECK_PARSING_STATUS (status, TRUE) ;

#define BYTE(a_parser, a_offset, a_eof) \
cr_tknzr_peek_byte2 (PARSER_PRIVATE (a_this)->tknzr, a_offset, a_eof)
...
status = cr_tknzr_read_byte (PARSER_PRIVATE (a_this)->tknzr, a_byte_ptr) ; \
CHECK_PARSING_STATUS (status, TRUE) ;
...
status = cr_tknzr_seek_index (PARSER_PRIVATE (a_this)->tknzr, \
...
status = cr_tknzr_consume_chars \
     (PARSER_PRIVATE (a_parser)->tknzr,0, &nb_chars) ; \
CHECK_PARSING_STATUS (status, TRUE) ; \
}
...
status = cr_tknzr_get_next_token (PARSER_PRIVATE (a_this)->tknzr, \
                                  a_token_ptr) ; \
                      ...
                      const guchar * a_msg, enum CRStatus a_status)
{
        ...
        g_return_val_if_fail (a_this && PARSER_PRIVATE (a_this)
        ...
        PARSER_PRIVATE (a_this)->err_stack =
                g_list_prepend (PARSER_PRIVATE (a_this)->err_stack, error);

        if (PARSER_PRIVATE (a_this)->err_stack == NULL)
                goto error;
...
cr_parser_dump_err_stack (CRParser * a_this, gboolean a_clear_errs)
{
        ...
        g_return_val_if_fail (a_this && PARSER_PRIVATE (a_this), CR_BAD_PARAM_ERROR);

        if (PARSER_PRIVATE (a_this)->err_stack == NULL)
                return CR_OK;

        for (cur = PARSER_PRIVATE (a_this)->err_stack; cur; cur = cur->next) {
...
cr_parser_clear_errors (CRParser * a_this)
{
        ...
        if (PARSER_PRIVATE (a_this)->err_stack) {
                g_list_free (PARSER_PRIVATE (a_this)->err_stack);
                PARSER_PRIVATE (a_this)->err_stack = NULL;
        }
...
cr_parser_try_to_skip_spaces_and_comments (CRParser * a_this)
{
        ...
        g_return_val_if_fail (a_this && PARSER_PRIVATE (a_this)
                              && PARSER_PRIVATE (a_this)->tknzr, CR_BAD_PARAM_ERROR);
        do {
                if (token) {
        ...
        cr_tknzr_unget_token (PARSER_PRIVATE (a_this)->tknzr, token);
...
cr_parser_parse_stylesheet_core (CRParser * a_this)
{
 ...
 continue_parsing:
        ...
        status = cr_tknzr_get_next_token (PARSER_PRIVATE (a_this)->tknzr, &token);
        ...
        case CDC_TK:
                ...
                break;
        default:
                status = cr_tknzr_unget_token (PARSER_PRIVATE (a_this)->tknzr,
                                               token);
                CHECK_PARSING_STATUS (status, TRUE);
                token = NULL;
 ...
 error:
        ...
        cr_tknzr_set_cur_pos (PARSER_PRIVATE (a_this)->tknzr, &init_pos);
...
cr_parser_parse_atrule_core (CRParser * a_this)
{
        ...
        if (token->type == CBO_TK) {
                cr_tknzr_unget_token (PARSER_PRIVATE (a_this)->tknzr, 
                                      token);
                token = NULL;
 ...
 error:
        ...
        cr_tknzr_set_cur_pos (PARSER_PRIVATE (a_this)->tknzr,
                              &init_pos);
        return status;
}
                          ...
                          guint      n_calls)
{
        ...
        status = cr_tknzr_get_next_token (PARSER_PRIVATE (a_this)->tknzr, &token1);
        ...
        case PO_TK:
                ...
                if (token2->type == PC_TK) {
                        ...
                        status = cr_tknzr_unget_token
                                (PARSER_PRIVATE (a_this)->tknzr, token2);
                        token2 = NULL;
                }
...
cr_parser_parse_simple_selector (CRParser * a_this, CRSimpleSel ** a_sel)
{
        ...
        while (cr_tknzr_get_nb_bytes_left ((PARSER_PRIVATE (a_this)->tknzr))) {
                if (token) {
                        cr_token_destroy (token);
                        token = NULL;
                }
                ...
                if (token && token->type == HASH_TK) {
                        ...
                        status = cr_tknzr_get_next_token
                                (PARSER_PRIVATE (a_this)->tknzr, &token);
                        if (status != CR_OK)
                                goto error;
                        ...
                        if (token->type == IDENT_TK) {
                                ...
                                status = cr_tknzr_unget_token (PARSER_PRIVATE (a_this)->tknzr, token);
                                token = NULL;
                             ...
                             CRSimpleSel ** a_sel)
{
        ...
        *a_sel = cr_simple_sel_append_simple_sel (*a_sel, sel);
        ...
        for (;;) {
                ...
                status =
                    cr_tknzr_peek_char (PARSER_PRIVATE (a_this)->tknzr, &next_char);
                if (status == CR_END_OF_INPUT_ERROR)
                        break;
                if (status != CR_OK)
                        goto error;
                          ...
                          CRSelector ** a_selector)
{
        ...
        status = cr_tknzr_peek_char (PARSER_PRIVATE (a_this)->tknzr,
                                     &next_char);
        if (status != CR_OK) {
                if (status == CR_END_OF_INPUT_ERROR) {
                        status = CR_OK;
...
cr_parser_parse_uri (CRParser * a_this, CRString ** a_str)
{
        ...
        status = cr_tknzr_parse_token (PARSER_PRIVATE (a_this)->tknzr,
...
cr_parser_parse_stylesheet (CRParser * a_this)
{
        ...
        PARSER_PRIVATE (a_this)->state = READY_STATE;

        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->start_document) {
                PARSER_PRIVATE (a_this)->sac_handler->start_document
                        (PARSER_PRIVATE (a_this)->sac_handler);
        }
 ...
 parse_charset:
        ...
        if (token && token->type == CHARSET_SYM_TK) {
                ...
                if (status == CR_OK && charset) {
                        if (PARSER_PRIVATE (a_this)->sac_handler
                            && PARSER_PRIVATE (a_this)->sac_handler->charset) {
                                PARSER_PRIVATE (a_this)->sac_handler->charset
                                        (PARSER_PRIVATE (a_this)->sac_handler,
                                         charset, &location);
                        }
...
/* parse_imports:*/
        ...
        for (;;) {
                ...
                if (token && token->type == IMPORT_SYM_TK) {
                        ...
                        if (status == CR_OK) {
                                if (import_string
                                    && PARSER_PRIVATE (a_this)->sac_handler
                                    && PARSER_PRIVATE (a_this)->sac_handler->import_style) {
                                        PARSER_PRIVATE (a_this)->sac_handler->import_style 
                                                (PARSER_PRIVATE(a_this)->sac_handler,
                                                 media_list,
                                                 import_string,
                                        ...
                                        if (PARSER_PRIVATE (a_this)->sac_handler->resolve_import == TRUE) {
                                                /*
                                                 *TODO: resolve the
                                                 *import rule.
                                                 */
                                        ...
                                        if (PARSER_PRIVATE (a_this)->sac_handler->import_style_result) {
                                                PARSER_PRIVATE (a_this)->sac_handler->import_style_result
                                                        (PARSER_PRIVATE (a_this)->sac_handler,
                                ...
                                if (PARSER_PRIVATE (a_this)->sac_handler
                                    && PARSER_PRIVATE (a_this)->sac_handler->error) {
                                        PARSER_PRIVATE (a_this)->sac_handler->error
                                                (PARSER_PRIVATE (a_this)->sac_handler);
                                }
                                status = cr_parser_parse_atrule_core (a_this);
 ...
 parse_ruleset_and_others:
        ...
        if (status == CR_END_OF_INPUT_ERROR || status == CR_OK) {
                ...
                if (PARSER_PRIVATE (a_this)->sac_handler
                    && PARSER_PRIVATE (a_this)->sac_handler->end_document) {
                        PARSER_PRIVATE (a_this)->sac_handler->end_document
                                (PARSER_PRIVATE (a_this)->sac_handler);
                }
        ...
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->unrecoverable_error) {
                PARSER_PRIVATE (a_this)->sac_handler->
                        unrecoverable_error (PARSER_PRIVATE (a_this)->sac_handler);
        }
...
cr_parser_new (CRTknzr * a_tknzr)
{
        ...
        PARSER_PRIVATE (result) = g_malloc0 (sizeof (CRParserPriv));
...
cr_parser_set_sac_handler (CRParser * a_this, CRDocHandler * a_handler)
{
        ...
        if (PARSER_PRIVATE (a_this)->sac_handler) {
                cr_doc_handler_unref (PARSER_PRIVATE (a_this)->sac_handler);
        }

        PARSER_PRIVATE (a_this)->sac_handler = a_handler;
...
cr_parser_get_sac_handler (CRParser * a_this, CRDocHandler ** a_handler)
{
        ...
        *a_handler = PARSER_PRIVATE (a_this)->sac_handler;
                                ...
                                gboolean a_use_core_grammar)
{
        ...
        PARSER_PRIVATE (a_this)->use_core_grammar = a_use_core_grammar;
                                ...
                                gboolean * a_use_core_grammar)
{
        ...
        *a_use_core_grammar = PARSER_PRIVATE (a_this)->use_core_grammar;
...
cr_parser_parse_ruleset (CRParser * a_this)
{
        ...
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->start_selector) {
                /*
                ...
                PARSER_PRIVATE (a_this)->sac_handler->start_selector
                        (PARSER_PRIVATE (a_this)->sac_handler, selector);
                start_selector = TRUE;
        }

        cr_parser_try_to_skip_spaces_and_comments (a_this);

        PARSER_PRIVATE (a_this)->state = TRY_PARSE_RULESET_STATE;
        ...
        if (status == CR_OK
            && PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->property) {
                PARSER_PRIVATE (a_this)->sac_handler->property
                        (PARSER_PRIVATE (a_this)->sac_handler, property, expr,
                         is_important);
        }        
        if (status == CR_OK) {
                /*
                 ...
                 */
                status = cr_tknzr_peek_char (PARSER_PRIVATE (a_this)->tknzr, &c) ;
                if (status == CR_OK && c == '}') {
                        status = CR_OK ;
                        goto end_of_ruleset ;
                }
 ...
 end_of_ruleset:
        ...
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->end_selector) {
                PARSER_PRIVATE (a_this)->sac_handler->end_selector
                        (PARSER_PRIVATE (a_this)->sac_handler, selector);
        ...
        cr_parser_clear_errors (a_this);
        PARSER_PRIVATE (a_this)->state = RULESET_PARSED_STATE;
                        ...
                        CRParsingLocation *a_location)
{
        ...
        PARSER_PRIVATE (a_this)->state = TRY_PARSE_IMPORT_STATE;
        ...
        for (; status == CR_OK;) {
                if ((status = cr_tknzr_peek_char (PARSER_PRIVATE (a_this)->tknzr,
                                                  &next_char)) != CR_OK) {
                        if (status == CR_END_OF_INPUT_ERROR) {
                                status = CR_OK;
                                goto okay;
      ...
      okay:
        cr_parser_clear_errors (a_this);
        PARSER_PRIVATE (a_this)->state = IMPORT_PARSED_STATE;
...
cr_parser_parse_media (CRParser * a_this)
{
         ...
         */
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->start_media) {
                PARSER_PRIVATE (a_this)->sac_handler->start_media
                        (PARSER_PRIVATE (a_this)->sac_handler, media_list,
                         &location);
        }

        cr_parser_try_to_skip_spaces_and_comments (a_this);

        PARSER_PRIVATE (a_this)->state = TRY_PARSE_MEDIA_STATE;
         ...
         */
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->end_media) {
                PARSER_PRIVATE (a_this)->sac_handler->end_media
                        (PARSER_PRIVATE (a_this)->sac_handler, media_list);
        }
        ...
        cr_parser_clear_errors (a_this);
        PARSER_PRIVATE (a_this)->state = MEDIA_PARSED_STATE;
...
cr_parser_parse_page (CRParser * a_this)
{
         ...
         */
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->start_page) {
                PARSER_PRIVATE (a_this)->sac_handler->start_page
                        (PARSER_PRIVATE (a_this)->sac_handler,
        ...
        PARSER_PRIVATE (a_this)->state = TRY_PARSE_PAGE_STATE;
         ...
         */
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->end_page) {
                PARSER_PRIVATE (a_this)->sac_handler->end_page
                        (PARSER_PRIVATE (a_this)->sac_handler,
                         page_selector, page_pseudo_class);
        }
        ...
        cr_parser_clear_errors (a_this);
        PARSER_PRIVATE (a_this)->state = PAGE_PARSED_STATE;
                         ...
                         CRParsingLocation *a_charset_sym_location)
{
        ...
        PARSER_PRIVATE (a_this)->state = TRY_PARSE_CHARSET_STATE;
        ...
        PARSER_PRIVATE (a_this)->state = CHARSET_PARSED_STATE;
        return CR_OK;
...
cr_parser_parse_font_face (CRParser * a_this)
{
         ...
         */
        if (PARSER_PRIVATE (a_this)->sac_handler
            && PARSER_PRIVATE (a_this)->sac_handler->start_font_face) {
                PARSER_PRIVATE (a_this)->sac_handler->start_font_face
                        (PARSER_PRIVATE (a_this)->sac_handler, &location);
        }
        PARSER_PRIVATE (a_this)->state = TRY_PARSE_FONT_FACE_STATE;
        /*
        ...
        for (;;) {
                 ...
                 */
                cr_term_ref (css_expression);
                if (PARSER_PRIVATE (a_this)->sac_handler->property) {
                        PARSER_PRIVATE (a_this)->sac_handler->property
                                (PARSER_PRIVATE (a_this)->sac_handler,
                                 property, css_expression, important);
                }
         ...
         *here, call the relevant SAC handler.
         */
        if (PARSER_PRIVATE (a_this)->sac_handler->end_font_face) {
                PARSER_PRIVATE (a_this)->sac_handler->end_font_face
                        (PARSER_PRIVATE (a_this)->sac_handler);
        }
        cr_parser_try_to_skip_spaces_and_comments (a_this);
        ...
        cr_parser_clear_errors (a_this);
        PARSER_PRIVATE (a_this)->state = FONT_FACE_PARSED_STATE;
        return CR_OK;
...
cr_parser_parse (CRParser * a_this)
{
        ...
        if (PARSER_PRIVATE (a_this)->use_core_grammar == FALSE) {
...
cr_parser_set_tknzr (CRParser * a_this, CRTknzr * a_tknzr)
{
        ...
        if (PARSER_PRIVATE (a_this)->tknzr) {
                cr_tknzr_unref (PARSER_PRIVATE (a_this)->tknzr);
        }

        PARSER_PRIVATE (a_this)->tknzr = a_tknzr;
...
cr_parser_get_tknzr (CRParser * a_this, CRTknzr ** a_tknzr)
{
        ...
        *a_tknzr = PARSER_PRIVATE (a_this)->tknzr;
        return CR_OK;
}
                                ...
                                CRParsingLocation *a_loc)
{
        ...
        return cr_tknzr_get_parsing_location 
                (PARSER_PRIVATE (a_this)->tknzr, a_loc) ;
}
...
{
        g_return_if_fail (a_this && PARSER_PRIVATE (a_this));

        if (PARSER_PRIVATE (a_this)->tknzr) {
                if (cr_tknzr_unref (PARSER_PRIVATE (a_this)->tknzr) == TRUE)
                        PARSER_PRIVATE (a_this)->tknzr = NULL;
        }

        if (PARSER_PRIVATE (a_this)->sac_handler) {
                cr_doc_handler_unref (PARSER_PRIVATE (a_this)->sac_handler);
                PARSER_PRIVATE (a_this)->sac_handler = NULL;
        }
        ...
        if (PARSER_PRIVATE (a_this)) {
                g_free (PARSER_PRIVATE (a_this));
                PARSER_PRIVATE (a_this) = NULL;
        }
