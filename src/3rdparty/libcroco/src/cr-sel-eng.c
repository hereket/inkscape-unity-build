...
#define SEL_PRIVATE(a_this) (a_this)->priv
...
cr_sel_eng_set_node_iface (CRSelEng *const a_this, CRNodeIface const *const a_node_iface)
{
        ...
        SEL_PRIVATE(a_this)->node_iface = a_node_iface;
}
                           ...
                           CRAdditionalSel * a_sel, CRXMLNodePtr const a_node)
{
        ...
        g_return_val_if_fail (a_this && SEL_PRIVATE (a_this)
        ...
        node_iface = SEL_PRIVATE(a_this)->node_iface;
                                      ...
                                      gulong * a_capacity)
{
         ...
         */
        if (SEL_PRIVATE (a_this)->sheet != a_stylesheet) {
                SEL_PRIVATE (a_this)->sheet = a_stylesheet;
                SEL_PRIVATE (a_this)->cur_stmt = a_stylesheet->statements;
        }
         ...
         *selectors lists.
         */
        for (cur_stmt = SEL_PRIVATE (a_this)->cur_stmt;
             (SEL_PRIVATE (a_this)->cur_stmt = cur_stmt);
         ...
         *anymore.
         */
        g_return_val_if_fail (!SEL_PRIVATE (a_this)->cur_stmt, CR_ERROR);
        SEL_PRIVATE (a_this)->sheet = NULL;
        return CR_OK;
}
...
cr_sel_eng_new (CRNodeIface const * a_node_iface)
{
        ...
        SEL_PRIVATE (result) = (CRSelEngPriv *) g_try_malloc (sizeof (CRSelEngPriv));
        if (!SEL_PRIVATE (result)) {
                cr_utils_trace_info ("Out of memory");
                g_free (result);
                return NULL;
        }
        memset (SEL_PRIVATE (result), 0, sizeof (CRSelEngPriv));
                                              ...
                                              a_handler)
{
        ...
        list = g_list_append (SEL_PRIVATE (a_this)->pcs_handlers, handler_entry);
        if (!list) {
                return CR_OUT_OF_MEMORY_ERROR;
        }
        SEL_PRIVATE (a_this)->pcs_handlers = list;
        return CR_OK;
}
                                                ...
                                                enum CRPseudoType a_type)
{
        ...
        g_return_val_if_fail (a_this && SEL_PRIVATE (a_this), CR_BAD_PARAM_ERROR);

        for (elem = SEL_PRIVATE (a_this)->pcs_handlers;
        ...
        SEL_PRIVATE (a_this)->pcs_handlers = g_list_delete_link
                (SEL_PRIVATE (a_this)->pcs_handlers, elem);
        entry = (struct CRPseudoClassSelHandlerEntry *) elem->data;
        if (entry->name) {
                g_free (entry->name);
                entry->name = NULL;
...
cr_sel_eng_unregister_all_pseudo_class_sel_handlers (CRSelEng * a_this)
{
        ...
        if (!SEL_PRIVATE (a_this)->pcs_handlers)
                return CR_OK;
        for (elem = SEL_PRIVATE (a_this)->pcs_handlers;
             elem; elem = g_list_next (elem)) {
                entry = (struct CRPseudoClassSelHandlerEntry *) elem->data;
        ...
        g_list_free (SEL_PRIVATE (a_this)->pcs_handlers);
        SEL_PRIVATE (a_this)->pcs_handlers = NULL;
        return CR_OK;
}
                         ...
                         CRXMLNodePtr a_node, gboolean * a_result)
{
        ...
        if (!SEL_PRIVATE(a_this)->node_iface->isElementNode (a_node)) {
                *a_result = FALSE;
                return CR_OK;
        }
...
cr_sel_eng_destroy (CRSelEng * a_this)
{
        ...
        g_free (SEL_PRIVATE (a_this));
        SEL_PRIVATE (a_this) = NULL;
 end:
        if (a_this) {
                g_free (a_this);
        }
