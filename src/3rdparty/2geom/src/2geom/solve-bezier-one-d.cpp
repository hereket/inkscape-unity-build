 ...
 **/
struct BernsteinOneD
{
    static constexpr size_t MAX_DEPTH = 53;
    size_t degree, N;
    std::vector<double> &solutions;

    BernsteinOneD(size_t _degree, std::vector<double> &sol)
        : degree{_degree}
        , N{degree + 1}
        , solutions{sol}
    {}
...
{
    BernsteinOneD B(degree, solutions);
...
find_bernstein_roots(std::vector<double> &solutions, /* RETURN candidate t-values */
                     ...
                     double left_t, double right_t)
{
    BernsteinOneD B(bz.degree(), solutions);
    Geom::Bezier& bzl = const_cast<Geom::Bezier&>(bz);
    double* w = &(bzl[0]);
    B.find_bernstein_roots(w, 0, left_t, right_t);
}

void BernsteinOneD::find_bernstein_roots(double const *w, /* The control points  */
                          unsigned depth,    /* The depth of the recursion */
                          double left_t,
                          double right_t)
{
