...
void Layout::fitToPathAlign(SVGLength const &startOffset, livarot::Path const &path)
{
    double offset = 0.0;

    if (startOffset._set) {
        if (startOffset.unit == SVGLength::PERCENT)
            offset = startOffset.computed * const_cast<livarot::Path&>(path).Length();
        else
    ...
    if (_characters.empty()) {
        int unused = 0;
        livarot::Path::cut_position *point_otp = const_cast<livarot::Path&>(path).CurvilignToPosition(1, &offset, unused);
        if (offset >= 0.0 && point_otp != nullptr && point_otp[0].piece >= 0) {
            Geom::Point point;
            Geom::Point tangent;
            const_cast<livarot::Path&>(path).PointAndTangentAt(point_otp[0].piece, point_otp[0].t, point, tangent);
    ...
    for (unsigned char_index = 0 ; char_index < _characters.size() ; ) {
        ...
        livarot::Path::cut_position *midpoint_otp = const_cast<livarot::Path&>(path).CurvilignToPosition(1, &midpoint_offset, unused);
        if (midpoint_offset >= 0.0 && midpoint_otp != nullptr && midpoint_otp[0].piece >= 0) {
            Geom::Point midpoint;
            Geom::Point tangent;
            const_cast<livarot::Path&>(path).PointAndTangentAt(midpoint_otp[0].piece, midpoint_otp[0].t, midpoint, tangent);

            if (start_offset >= 0.0 && end_offset >= 0.0) {
                livarot::Path::cut_position *start_otp = const_cast<livarot::Path&>(path).CurvilignToPosition(1, &start_offset, unused);
                if (start_otp != nullptr && start_otp[0].piece >= 0) {
                    livarot::Path::cut_position *end_otp = const_cast<livarot::Path&>(path).CurvilignToPosition(1, &end_offset, unused);
                    if (end_otp != nullptr && end_otp[0].piece >= 0) {
                        ...
                        if (on_same_subpath) {
                            ...
                            const_cast<livarot::Path&>(path).PointAt(start_otp[0].piece, start_otp[0].t, startpoint);
                            const_cast<livarot::Path&>(path).PointAt(end_otp[0].piece, end_otp[0].t, endpoint);
