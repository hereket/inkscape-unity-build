...
Layout::iterator Layout::getNearestCursorPositionTo(double x, double y) const
{
    ...
    if (_path_fitted) {
        livarot::Path::cut_position position = const_cast<livarot::Path*>(_path_fitted)->PointToCurvilignPosition(Geom::Point(x, y));
        local_x = const_cast<livarot::Path*>(_path_fitted)->PositionToLength(position.piece, position.t);
...
Geom::Rect Layout::characterBoundingBox(iterator const &it, double *rotation) const
{
    ...
    if (_path_fitted) {
        ...
        int unused = 0;
        livarot::Path::cut_position *midpoint_otp = const_cast<livarot::Path*>(_path_fitted)->CurvilignToPosition(1, &midpoint_offset, unused);
        if (midpoint_offset >= 0.0 && midpoint_otp != nullptr && midpoint_otp[0].piece >= 0) {
            Geom::Point midpoint;
            Geom::Point tangent;
            Span const &span = _characters[char_index].span(this);

            const_cast<livarot::Path*>(_path_fitted)->PointAndTangentAt(midpoint_otp[0].piece, midpoint_otp[0].t, midpoint, tangent);
...
void Layout::queryCursorShape(iterator const &it, Geom::Point &position, double &height, double &rotation) const
{
    if (_characters.empty()) {
        ...
        if (_path_fitted) {
            ...
            double path_length = const_cast<livarot::Path*>(_path_fitted)->Length();
            double x_on_path = x;
            if (x_on_path < 0.0) x_on_path = 0.0;

            int unused = 0;
            // as far as I know these functions are const, they're just not marked as such
            livarot::Path::cut_position *path_parameter_list = const_cast<livarot::Path*>(_path_fitted)->CurvilignToPosition(1, &x_on_path, unused);
            livarot::Path::cut_position path_parameter;
            ...
            Geom::Point point;
            Geom::Point tangent;
            const_cast<livarot::Path*>(_path_fitted)->PointAndTangentAt(path_parameter.piece, path_parameter.t, point, tangent);
            if (x < 0.0)
                point += x * tangent;
            if (x > path_length )
