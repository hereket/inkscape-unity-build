 ...
 */
static Shape make_shape(livarot::Path &path, int path_id = -1, FillRule fill_rule = fill_nonZero, bool close_if_needed = true)
{
    Shape result;
 ...
 */
static livarot::Path make_path(Geom::PathVector const &pathv, std::vector<Geom::PathVectorTime> const &cuts)
{
    livarot::Path result;
 ...
 */
static bool is_line(livarot::Path const &path)
{
...
Geom::PathVector sp_pathvector_boolop(Geom::PathVector const &pathva, Geom::PathVector const &pathvb, BooleanOp bop, FillRule fra, FillRule frb)
{
    ...
    if (bop == bool_op_inters || bop == bool_op_union || bop == bool_op_diff || bop == bool_op_symdiff) {
        ...
        std::vector<livarot::Path::cut_position> toCut;
...
void Inkscape::ObjectSet::_pathBoolOp(BooleanOp bop)
{
    ...
    struct Operand
    {
        ...
        std::unique_ptr<livarot::Path> path;
    };
    ...
    for (auto &operand : operands) {
        ...
        operand.path = std::make_unique<livarot::Path>();
    ...
    livarot::Path::cut_position  *toCut=nullptr;
    int                  nbToCut=0;
                ...
                for (int i = 0; i < theShape->numberOfPoints(); i++) {
                    if ( theShape->getPoint(i).totalDegree() > 2 ) {
                        ...
                        if ( nbOrig > 0 && nbOther > 0 ) {
                            ...
                            // note that you only keep one position on the source; you could have degenerate
                            // cases where the source crosses itself at this point, and you wouyld miss an intersection
                            toCut=(livarot::Path::cut_position*)realloc(toCut, (nbToCut+1)*sizeof(livarot::Path::cut_position));
    ...
    auto get_path_arr = [&] {
        std::vector<livarot::Path *> result;
