...
SplinePoint *SplinePointCreate(real x, real y) {
    SplinePoint *sp;
    if ( (sp=(SplinePoint*)chunkalloc(sizeof(SplinePoint)))!=NULL ) {
	sp->me.x = x; sp->me.y = y;
...
Spline *SplineMake2(SplinePoint *from, SplinePoint *to) {
    Spline *spline = (Spline*)chunkalloc(sizeof(Spline));
...
FitPoint *SplinesFigureFPsBetween(SplinePoint *from, SplinePoint *to,
    ...
    if ( cnt>10 ) {
	lens = (bigreal*)malloc(cnt*sizeof(bigreal));
	cnts = (int*)malloc(cnt*sizeof(int));
    }
    cnt = 0; len = 0;
    for ( np = from->next->to; ; np = np->next->to ) {
    ...
    fp = (FitPoint*)malloc((pcnt+1)*sizeof(FitPoint)); i = 0;
    if ( len==0 ) {
	for ( i=0; i<=pcnt; ++i ) {
	    fp[i].t = i/(pcnt);
	    fp[i].p.x = from->me.x;
...
static enum pointtype SplinePointDowngrade(int current, int geom) {
	enum pointtype np = (enum pointtype)current;
...
int _SplinePointCategorize(SplinePoint *sp, int flags) {
	...
	geom = (pointtype)SplinePointCategory(sp);
