...
std::unique_ptr<livarot::Path> Path_for_pathvector(Geom::PathVector const &pathv)
{
    auto dest = std::make_unique<livarot::Path>();
    dest->LoadPathVector(pathv);
    return dest;
}

std::unique_ptr<livarot::Path> Path_for_item(SPItem *item, bool doTransformation, bool transformFull)
{
...
std::unique_ptr<livarot::Path> Path_for_item_before_LPE(SPItem *item, bool doTransformation, bool transformFull)
{
...
std::optional<livarot::Path::cut_position> get_nearest_position_on_Path(livarot::Path *path, Geom::Point p, unsigned seg)
{
    if (!path) {
        return {};
    }
...
Geom::Point get_point_on_Path(livarot::Path *path, int piece, double t)
{
    Geom::Point p;
