 ...
 * @return A pointer to the livarot Path object.
 */
std::unique_ptr<livarot::Path> Path_for_item(SPItem *item, bool doTransformation, bool transformFull = true);
 ...
 * @return A pointer to the livarot Path object.
 */
std::unique_ptr<livarot::Path> Path_for_item_before_LPE(SPItem *item, bool doTransformation, bool transformFull = true);
 ...
 */
std::optional<livarot::Path::cut_position> get_nearest_position_on_Path(livarot::Path *path, Geom::Point p, unsigned seg = 0);
