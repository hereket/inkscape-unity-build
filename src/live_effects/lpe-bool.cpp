...
bool cmp_cut_position(const livarot::Path::cut_position &a, const livarot::Path::cut_position &b)
{
...
sp_pathvector_boolop_slice_intersect(Geom::PathVector const &pathva, Geom::PathVector const &pathvb, bool inside, FillRule fra, FillRule frb)
{
    ...
    // Result path
    livarot::Path *result_path = new livarot::Path;
            ...
            for (int i = 0; i < combined_inters->numberOfPoints(); i++) {
                if (combined_inters->getPoint(i).totalDegree() > 2) {
                    ...
                    if (nbOrig > 0 && nbOther > 0) {
                        ...
                        livarot::Path::cut_position cutpos;
                        cutpos.piece = piece;
                        cutpos.t = t;
    ...
    std::vector<livarot::Path::cut_position> inside_pieces(combined_inters->numberOfEdges());
    ...
    {
        int idIncr = 0;
        std::vector<livarot::Path::cut_position>::iterator itPiece = inside_pieces.begin();
        std::vector<livarot::Path::cut_position>::iterator itCut = toCut.begin();
