...
static const Util::EnumDataConverter<unsigned> LpeJoinTypeConverter(JoinType, sizeof (JoinType)/sizeof(*JoinType));
...
LPETaperStroke::LPETaperStroke(LivePathEffectObject *lpeobject) :
    ...
    join_type(_("Join type:"), _("Join type for non-smooth nodes"), "jointype", LpeJoinTypeConverter, &wr, this, JOIN_EXTRAPOLATE),
...
LPETaperStroke::doBeforeEffect (SPLPEItem const* lpeitem)
{
    ...
    for (auto path : pathv) {
        ...
        if (!metInMiddle) {
            ...
            throwaway_path = half_outline(pathv_tmp[1], fabs(line_width)/2., miter_limit, static_cast<Inkscape::LineJoinType>(join_type.get_value()));
        ...
        if (!metInMiddle) {
            ...
            throwaway_path = half_outline(pathv_tmp[1].reversed(), fabs(line_width)/2., miter_limit, static_cast<Inkscape::LineJoinType>(join_type.get_value()));
