...
*/
// NOTE(alfred)
static std::optional<Point> lpe_intersection_point( Point const & origin_a, Point const & vector_a,
...
// NOTE(alfred):
static const Util::EnumData<unsigned> LpeInterpolatorTypeData[] = {
...
static const Util::EnumDataConverter<unsigned> LpeInterpolatorTypeConverter(LpeInterpolatorTypeData, sizeof(LpeInterpolatorTypeData)/sizeof(*LpeInterpolatorTypeData));
...
LPEPowerStroke::LPEPowerStroke(LivePathEffectObject *lpeobject) :
    ...
    interpolator_type(_("Smoothing type"), _("Determines which kind of interpolator will be used to interpolate between stroke width along the path"), "interpolator_type", LpeInterpolatorTypeConverter, &wr, this, Geom::Interpolate::INTERP_CENTRIPETAL_CATMULLROM),
        ...
        { // discontinuity found, so fix it :-)
            ...
            if (on_outside) {
                ...
                case LINEJOIN_ROUND: {
                    ...
                    std::optional<Geom::Point> O = lpe_intersection_point( B[prev_i].at1(), tang1,
                                                                              B[i].at0(), tang2 );
                    if (!O) {
                    ...
                    if (solutions.size() == 2) {
                        ...
                        if (solok) {
                            ...
                            if (arc0) {
                                ...
                                std::optional<Geom::Point> p = lpe_intersection_point( B[prev_i].at1(), tang1,
                                                                                B[i].at0(), tang2 );
                                if (p) {
                                    // check size of miter
                                    Geom::Point point_on_path = B[prev_i].at1() - rot90(tang1) * width;
